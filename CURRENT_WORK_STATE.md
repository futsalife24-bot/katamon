...
- ブランチ: `master`
- 現行ビルド: `v112`（起動演出の作り込み:山なりの軌道・崩壊の作り直し・傷みの表現。**未公開**／ルール変更なし）
...
- 現在の進行: 1台での2vs2は完走・再戦とも実機で確認済み。v109〜v112 でUIと起動演出に手を入れた。残る未検証は**2台以上での受け取り側**。次は 段D 切断・CPU引き継ぎ #8 (v113)。
...
> ルールの変更なし。Console作業は不要。ユーザーの指摘3点への対応。
