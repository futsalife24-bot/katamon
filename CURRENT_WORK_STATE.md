...
## v147 モンスター選択カードの木板デザイン（マージ承認済み・実ブラウザ確認待ち）

- 作業ブランチ: `feat/v147-character-card-wood-design`。最新版master（v146・PR #84まで）から作成し、記録専用のPR #83には積み増していない。
...
- 見た目確認: ローカルHTTPで2回確認を試みたが、この作業環境から接続できる実ブラウザが0件で、スマホ幅での文字切れ・重なり・木板の見え方はまだ確認できていない。メロニキから2026-08-09に「マージまで」と明示の指示があったため、自動検査全件成功と中継数不変を確認したうえで公開へ進める。**公開後も実機で文字切れ・重なり・木板の見え方を確認すること。**
- `database.rules.json` は変更なし。Firebase Console作業なし。

## v146 防御力・通常弾性能の統一（承認待ち・PR #84）
...
24. **v146 防御力・通常弾性能の統一** — 承認待ち（PR #84）。実装・検査の詳細は冒頭のv146節を正本とする。
25. **v147 モンスター選択カードの木板デザイン** — マージ承認済み・実ブラウザ確認待ち。詳細は冒頭のv147節を正本とする。
26. **v148 必殺カットイン音をスタイリッシュな音へ変更** — 未着手。
   - v144と同じく、必要ならPixabayから音源を取得し、配布元URLとライセンス確認日を記録する。
27. Issue #5 対戦部屋内の戦績表示
...
29. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
