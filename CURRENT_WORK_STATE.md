...
## v153 演習の設定を戦闘中へ移動し、キャラ画像を戻す（PR作成前）
...
- 実画面: ローカルHTTP＋実ブラウザを試したが、この作業環境で接続可能なブラウザは0台だった。ローカルのPlaywright実行環境も無いため、PRのモバイル実ブラウザ検査で画面遷移を確認する。ここでは実機確認済みとは扱わない。
...
35. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
