...
- ブランチ: `master`
- 現行ビルド: `v107`（CPUだけの相手だと再戦が自動で始まらない不具合の修正。**未公開**／ルール変更なし）
- 直前の公開ビルド: `v106`（CPUだけの相手で35秒で切られる不具合の修正）
- 直前の公開ビルド: `v105`（v104の実機で見つかった3件の修正。**CPUのみの2vs2が35秒で切れる → v106で修正**）
- 直前の公開ビルド: `v104`（Issue #25 段B ＋ Issue #26 段C オンライン2vs2。Consoleルール反映済み。**2vs2を選べない不具合あり → v105で修正済み**）
- 直前の公開ビルド: `v102`（Issue #23 すぐ対戦のマッチメイキング。**実機QAは1vs1を1試合回したのみ**）
...
- 現在の進行: v106 で**1人＋CPU3体の2vs2が完走できることを実機で確認**（ユーザー報告「概ね大丈夫そう」）。残った「再戦がやや遅い」を v107 で修正。次は 段D 切断・CPU引き継ぎ #8 (v108)。
...
### まだ確認できていないこと（実機QAで・優先度順）

**自動テストは受信経路と判定関数までで、4台が同時につながった状態は一度も通していない。**
`loopbacktest.js` は2人ぶん・v2プロトコルなので、ここは埋まっていない。

1. **2人で2vs2（残り2席はCPU）。** 今回いちばん新しく、いちばん壊れやすい
   「ホストが空席のCPUを動かして結果を配る」経路が、ここに全部乗る。最初に踏むこと。
2. 空席のCPUが撃つところが、ホスト以外の画面でも同じ弾道・同じダメージか。
3. 陣営全滅での決着と、4人ぶんの再戦。
4. 4体ぶんのHPバーとターンバーの見え方。
5. 3人＋CPU1体、および4人そろった2vs2。
...
3. ~~**v103 段B：4人の席とロビー**~~ ／ ~~**v104 段C：オンライン2vs2**~~ — 公開済み（ルール反映済み）。**実機QAのみ残り。**
   - 4台が同時につながった状態は自動テストで一度も通していない。ここが唯一の未確認領域。
