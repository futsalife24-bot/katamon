...
- ブランチ: `master`
- 作業中ブランチ: `feat/v119-vs-cutin`（VSカットイン。未公開）
...
- 現在の進行: v119 のVSカットインを実装し、顔位置の個別補正とローカル実ブラウザ確認まで完了。次はユーザーQAで見つかったロビーの幽霊席（表示では「応答なし」なのに空けられない）を別版で直し、**タブ2つでオンライン2vs2の受け取り側**を通す。段D 切断・CPU引き継ぎ #8 はその後へ繰り下げる（2026-08-04）。
...
4. **v119 バトル開始時のVSカットイン** — 実装済み・未公開。実ブラウザの見た目確認が残り。
5. **タブ2つでオンライン2vs2を通す**
   - ホストが配る側だけでなく、ゲストが受け取る側の表示・手番・HP・決着・再戦を確認する。
   - これは新しい版ではなく、段Cの最大の未検証点を埋めるQA。
6. **v120 段D / Issue #8 切断・CPU引き継ぎ**
   - 落ちた人の席をCPUが引き継ぎ、試合を最後まで終わらせる（決定2）。
7. Issue #5 対戦部屋内の戦績表示
...
9. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
