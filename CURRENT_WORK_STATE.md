...
- ブランチ: `master`
- 作業中ブランチ: `feat/v134-character-select-abilities`（PR #60。メロニキ承認前にマージしない）
- 現行ビルド: `v133`（次の風を1区間先まで予報。PR #59。公開済み）
- 次の候補: `v134`（キャラクター選択を必殺技表示だけに整理。作業中・未公開）
- ひとつ前: `v132`（タイトル画面の静止部分を1枚に焼く。PR #58）
- その前: `v131`（キャンバスを画面の実画素ぴったりに。PR #56）
- さらに前: `v130`（起動時に読む絵をWebPへ。PR #55）
...
- 現在の進行: **v133「次の風予報」まで公開済み。v134「キャラクター選択を必殺技だけに整理」は PR #60 で承認待ち・未公開。**
  v134候補は追加6件を含む合計 **1131件成功**。メロニキ承認待ちで止める。
  v134の次は、差し込み前の順へ戻り Issue #5（対戦部屋内の戦績表示）→ Issue #6（ランキング改修）→ Issue #4（オンラインHPバーのずれ）。
...
## v134 で入れる変更（キャラクター選択を必殺技だけに整理）

> ⏳ **未公開・PR #60 承認待ち**（作業ブランチ `feat/v134-character-select-abilities`）。ルール変更・Console作業は不要。
...
17. **v134 キャラクター選択を必殺技だけに整理** — メロニキの指示で差し込み。PR #60 承認待ち・未公開。
   - 型・紹介文・性能目盛り・必殺技説明を隠し、キャラ名と必殺技名だけにする。
   - 対戦の能力値は変えない。全1131件成功。
18. Issue #5 対戦部屋内の戦績表示
...
20. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
