...
- ブランチ: `master`
- 作業中ブランチ: `claude/katamon-v121-turn-tempo`（v121 手番表示とテンポ。未公開）
- 現行ビルド: `v121`（手番が誰か分かるようにし、テンポを落ち着かせる。未公開／ルール変更なし）
- 直前の公開ビルド: `v120`（ロビーの幽霊席の削除可否表示。PR #44）
...
- 現在の進行: v119・v120 は公開済み。v121 として、実機報告のあった「開始カットイン中に撃たれる」「誰の手番か分からない」「テンポが速い」を直した（未公開・ルール変更なし）。指定の全テストは合計 **869件成功**。次は 段D / Issue #8（切断・CPU引き継ぎ）を v122 として入れる。
...
## v121 で入れた変更（手番が誰か分かるようにし、テンポを落ち着かせる）
...
- 指定の全テストを実際に走らせ、合計 **869件すべて成功**（v120の842件から+27）。
  - `seattest`: 20件 × 2席
...
- `loopbacktest` の中継メッセージ数は `50 / 78 / 124 / 90 / 57`。
  **v120の `58 / 91 / 124 / 90 / 58` から変わっているが、これは想定どおり。**
...
7. **v121 手番が誰か分かるようにし、テンポを落ち着かせる** — 実装済み・未公開。ルール変更なし。
   - 開始カットイン中に撃たれる不具合の修正を含む（実機報告・2026-08-05）。
8. **v122 段D / Issue #8 切断・CPU引き継ぎ** — 次に入れる。
   - 落ちた人の席をCPUが引き継ぎ、試合を最後まで終わらせる（決定2）。
   - 2vs2のみ。ホスト自身が落ちた場合と1vs1は範囲外（ユーザー判断・2026-08-05）。
   - **`database.rules.json` の変更を伴う。公開前にFirebase Consoleへ反映すること。**
9. Issue #5 対戦部屋内の戦績表示
...
11. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
