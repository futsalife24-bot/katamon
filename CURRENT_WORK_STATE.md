...
- ブランチ: `master`
- 現行ビルド: `v105`（v104の実機で見つかった3件の修正。**未公開**）
- 直前の公開ビルド: `v104`（Issue #25 段B ＋ Issue #26 段C オンライン2vs2。Consoleルール反映済み。**2vs2を選べない不具合あり → v105で修正**）
...
- 現在の進行: v104（段B＋段C）を公開したが、実機で**対戦方式を2vs2に変えられない**不具合が見つかり v105 で修正。ルールの追加変更は不要。次は 段D 切断・CPU引き継ぎ #8 (v106)。
