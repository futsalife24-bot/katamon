...
- ブランチ: `master`
- 現行ビルド: `v111`（タイトルが震え続ける不具合の修正。**未公開**／ルール変更なし）
...
- 現在の進行: 1台での2vs2は完走・再戦とも実機で確認済み。v109〜v111 でUIと起動演出に手を入れた。残る未検証は**2台以上での受け取り側**。次は 段D 切断・CPU引き継ぎ #8 (v112)。
...
> ルールの変更なし。Console作業は不要。v110の不具合修正。
