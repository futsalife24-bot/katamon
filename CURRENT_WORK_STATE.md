...
- ブランチ: `master`
- 作業中ブランチ: `feat/v137-special-aura`（必殺のオーラ→カットイン→発射。PR #63、レビュー待ち）
- 現行ビルド: `v136`（目玉の電磁波の命中表示と移動不可エフェクト。PR #62。公開済み）
- 次に公開するビルド: `v137`（必殺のオーラ→カットイン→発射。実装・検証済み）
- 次の候補: `v138`（通常弾の着弾爆発音をPixabay音源へ変更）→ `v139`（必殺カットイン音を強化）
...
- 現在の進行: **v137「必殺のオーラ→カットイン→発射」を実装・検証済み。PR #63、レビュー待ち。**
  v137は追加6件（2席で新規3件ずつ）を含む合計 **1153件成功**。
  v137の次は v138（通常弾の着弾爆発音）→ v139（必殺カットイン音）。その後、各キャラの必殺技調整の相談を続ける。差し込み前の残タスク順は Issue #5（対戦部屋内の戦績表示）→ Issue #6（ランキング改修）→ Issue #4（オンラインHPバーのずれ）。
...
- 作業ブランチは `feat/content-studio-mvp`。ゲーム本体の `master` へ直接反映しない。
- `0.3.0` で「画像、向きと目、生成、キャラ、GitHub」の5ステップへ縮小した。能力・必殺技・プリセット選択・モーション調整スライダーは日常UIから外した。
- 画像1枚から前進、後退、単発砲撃、被弾、着地の5クリップを固定設定で一括生成する。前後移動は元の向きを保ち、被弾は目がある場合だけサイズ調整可能な×印を重ねる。
- 接地点、砲口、最大2つの目を通常の端末内画像処理で推測し、ズレた位置は画像タップで修正する。生成AI、LLM、外部画像処理APIは使わない。
- 入力画像は展開前に最大辺1600pxへ安全に縮小し、高品質512pxへ一度だけ正規化する。Worker未対応時だけ256px軽量代替を明示する。画像Canvas上からも縦スクロールできる。
- 「5種類をまとめて生成」はスクロール位置に関係なく下部ナビ上へ固定する。処理中は同じ場所に進捗と中止ボタンを表示する。
- 表示名と小文字ASCII IDだけで登録し、通常技は標準弾、必殺技は未設定かつボタン無効。生成物は5組のスプライトPNG・メタデータJSON、モーションプロファイルJSON、まとめたZIP。
- GitHubは専用ブランチへの1コミットとPRを自動化。「PRだけ」または「CI成功後に確認してマージ」を選べる。CI失敗、競合、head SHA不一致ならマージを中断する。GitHub未設定でもモックで完走できる。
- 型検査、85件の単体・結合テスト、クライアント／サーバービルド、Android 13相当（412×915、touch、DPR 2.625）のE2Eが成功。提供画像もコミット・外部送信せずE2E入力として完走した。既存ゲームの回帰はp1/e1各256件をそれぞれ3回連続で成功。浮島を拾って稀に落ちていた既存の地形検査は、最下層の実グリッドだけを測るよう安定化した。物理Android実機のPWAインストール、OS共有ターゲット、実GitHub認証は未確認。
...
20. **v137 必殺のオーラ→カットイン→発射** — メロニキの指示で差し込み。実装・検証済み、PR #63でレビュー待ち。
   ...
   - `database.rules.json` は変更なし。Firebase Console作業なし。
21. **v138 通常弾の着弾爆発音をPixabay音源へ変更** — 未着手。
   - 音源本体をローカルへ保存し、配布元URLとライセンス確認日を記録する。オフラインでも鳴るようサービスワーカーの対象へ含める。
22. **v139 必殺カットイン音をスタイリッシュな音へ変更** — 未着手。
   - v138と同じく、必要ならPixabayから音源を取得し、配布元URLとライセンス確認日を記録する。
23. Issue #5 対戦部屋内の戦績表示
...
25. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
