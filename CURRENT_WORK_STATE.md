...
- 現在の進行: **v131 まで公開済み。未コミットの作業は無い。** 指定の全テストは合計 **1090件成功**。
  次は軽量化の最後の1つ（v132 タイトル画面を焼く）。その後 Issue #5（対戦部屋内の戦績表示）。
...
15. **軽量化（4版に分ける）** — ユーザー要望・2026-08-05。実機で「動作が重い」。
    ...
    - **次（最後）: v132 タイトル画面を焼く。`drawTitleScreen` が1コマ52ミリ秒で、タイトルの半分以上。**
      実機で「まだタイトルから重い」と言われている（2026-08-05）。やり方はv129と同じ。
