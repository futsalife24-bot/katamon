...
- ブランチ: `master`
- 作業中ブランチ: `claude/katamon-handoff-avsydw`（v129 描き直しの節約。未公開）
- 現行ビルド: `v129`（空と地形を1枚に焼いて貼る。未公開／ルール変更なし）
- ひとつ前: `v128`（撃破済みへのダメージ表示／起動演出の砲弾の絵。PR #53）
- その前: `v127`（チュートリアル最後の足場／VSカットの表示時間。PR #52）
- さらに前: `v126`（チュートリアル「あそび方」。PR #51）
...
- 現在の進行: v128 まで公開済み。その上に v129 として、実機で「動作が重い」という報告を受けて描画の作り直しを入れた（未公開・ルール変更なし）。指定の全テストは合計 **1057件成功**。軽量化は3版に分けて進める（v129=描き方、次=画像の圧縮、その次=解像度の上限）。その後 Issue #5（対戦部屋内の戦績表示）。
...
15. **軽量化（3版に分ける）** — ユーザー要望・2026-08-05。実機で「動作が重い」。
    - v129 描き方の作り直し（空と地形を1枚に焼く）— 実装済み・未公開。
    - 次: 画像の圧縮（PNG 4.48MB→0.80MB）。
    - その次: 解像度の上限（dprを2で頭打ち）。実機で見た目を確かめてから決める。
