...
- ブランチ: `master`
- 作業中ブランチ: `claude/katamon-handoff-avsydw`（v123 VSカットインの砲弾ネームプレート。未公開）
- 現行ビルド: `v123`（VSカットインを砲弾ネームプレートへ。未公開／ルール変更なし）
- 直前の公開ビルド: `v122`（手番表示とテンポ。PR #47）
...
- 現在の進行: v122 まで公開済み。その上に v123 として、対戦開始のVSカットインをユーザー提供の砲弾ネームプレートへ作り替えた（未公開・ルール変更なし）。指定の全テストは合計 **927件成功**。次は Issue #5（対戦部屋内の戦績表示）。
...
9. **v123 VSカットインを砲弾ネームプレートへ** — 実装済み・未公開。ルール変更なし。
   - 素材はユーザー提供。左右から砲弾が飛んできて中央でぶつかる（実機要望・2026-08-05）。
10. Issue #5 対戦部屋内の戦績表示
...
12. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
