...
- ブランチ: `master`
- 作業中ブランチ: なし（v119・v120はどちらもmasterへマージ済み）
- 現行ビルド: `v120`（ロビーの幽霊席の削除可否表示。PR #44 でmasterへマージ済み／ルール変更なし）
- 直前の公開ビルド: `v119`（バトル開始時のVSカットイン。PR #43）
...
- 現在の進行: v119（VSカットイン）と v120（ロビーの幽霊席）を公開済み。指定の全テストは合計 **842件成功**（2026-08-05 に再実行して確認）。次は v121 として 段D / Issue #8「切断・CPU引き継ぎ」に着手する。
