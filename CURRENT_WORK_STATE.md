...
- ブランチ: `master`
- 現行ビルド: `v108`（401でやり直さない不具合の修正＋「CPUで始める」廃止。**未公開**／ルール変更なし）
- 直前の公開ビルド: `v107`（CPUだけの相手だと再戦が自動で始まらない不具合の修正）
...
- 現在の進行: 1人＋CPU3体の2vs2は完走できる。v108 で「認証切れで再戦できない」を修正し、部屋の導線を整理（「CPUで始める」を廃止）。次は 段D 切断・CPU引き継ぎ #8 (v109)。
