...
- ブランチ: `master`
- 現行ビルド: `v116`（キャラ画像にキャッシュ版数を付ける。**未公開**／ルール変更なし）
- 直前の公開ビルド: `v115`（ニセンモノの画像差し替え。**古い絵が出続ける問題あり → v116で修正**）
...
- 現在の進行: 1台での2vs2は完走・再戦とも実機で確認済み。v109〜v116 でUI・起動演出・キャラ画像に手を入れた。残る未検証は**2台以上での受け取り側**。次は 段D 切断・CPU引き継ぎ #8 (v117)。
...
> ルールの変更なし。Console作業は不要。v115の不具合修正。
