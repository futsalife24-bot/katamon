...
- ブランチ: `master`
- 作業中ブランチ: `claude/katamon-handoff-avsydw`（v131 描く細かさを画面に合わせる。未公開）
- 現行ビルド: `v131`（キャンバスを画面の実画素ぴったりにする。未公開／ルール変更なし）
- ひとつ前: `v130`（起動時に読む絵をWebPへ。PR #55）
- その前: `v129`（空と地形を1枚に焼いて貼る。PR #54）
- さらに前: `v128`（撃破済みへのダメージ表示／起動演出の砲弾の絵。PR #53）
...
- 現在の進行: v130 まで公開済み。その上に v131 として、キャンバスの大きさを画面の実画素に合わせた（未公開・ルール変更なし）。指定の全テストは合計 **1090件成功**。軽量化は v129=描き方 → v130=画像 → v131=描く細かさ → 残り1つ=タイトル画面を焼く。その後 Issue #5（対戦部屋内の戦績表示）。
...
15. **軽量化（4版に分ける）** — ユーザー要望・2026-08-05。実機で「動作が重い」。
    ...
    - v131 描く細かさを画面に合わせる（キャンバスを実画素ぴったりに）— 実装済み・未公開。1.4〜1.8倍速。
    - 次（最後）: **タイトル画面を焼く。`drawTitleScreen` が1コマ52ミリ秒で、タイトルの半分以上。**
