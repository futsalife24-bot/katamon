...
## v163 2vs2ロビーの席を自分基準で整列（未公開・PR #106）

- 作業ブランチ: `fix/v163-lobby-seat-order`。公開済みv162（PR #105）への実機指摘だけを直し、PR #106を作成した。
...
- 表示確認: ローカルHTTPで`index.html`と`sw.js`がともに`200`、v163の番号を返すことを確認した。このセッションで接続可能な実ブラウザは0台のため直接目視はできていない。PRの`mobile-e2e`でスマホ幅の実ブラウザ検査を通してから公開する。
