...
## v156 標準／大型ステージの選択式（実装完了・Firebase Console反映待ち）

- 作業ブランチ: `feat/v156-stage-size-select`。v155のmasterから開始。PR作成後、GitHub Actions成功とFirebase Console反映を確認してからmasterへマージする。
...
- **Firebase Console作業が必要**: `database.rules.json`へ`settings.stageSize`の許可と検証を追加した。公開（masterマージ）前に、Firebase ConsoleのRealtime Database Rulesへこのファイル内容を反映・公開すること。反映前はオンラインで大型を選んだ設定が拒否される。
...
34. **v156 標準／大型ステージの選択式** — 実装完了。Firebase Console反映とPR公開待ち。
   - 大型は高さを活かす専用の出撃位置・カメラ範囲・Stage Studio生成／検査をまとめて扱い、通常サイズと選べるようにする。
35. Issue #5 対戦部屋内の戦績表示
...
37. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
