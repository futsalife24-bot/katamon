...
- ブランチ: `master`
- 作業中ブランチ: `perf/v132-title-art`（v132、PR #58。メロニキ承認前のためmasterへは未マージ）
- 現行ビルド: `v131`（キャンバスを画面の実画素ぴったりに。PR #56。公開済み）
...
- 現在の進行: **公開版はv131。v132「タイトル画面を焼く」はPR #58でメロニキ承認待ち。**
  指定の全テストは追加8件を含む合計 **1098件成功**。メロニキの承認前なのでmasterへは未マージ。
  その後は Issue #5（対戦部屋内の戦績表示）。
...
> ⏳ **PR #58でメロニキ承認待ち。** 公開版はまだv131。ルール変更・Console作業は不要。
...
15. **軽量化（4版に分ける）** — ユーザー要望・2026-08-05。実機で「動作が重い」。
    ...
    - ~~v132 タイトル画面を焼く~~ — PR #58でメロニキ承認待ち。2倍解像度で1枚に焼き、
      画素比較は完全一致。追加8件を含む全1098件成功。メロニキ承認前のため未マージ。
