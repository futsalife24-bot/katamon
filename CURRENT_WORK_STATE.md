...
- ブランチ: `master`
- 現行ビルド: `v102`（Issue #23 すぐ対戦のマッチメイキング。master へマージ済み／Consoleルール反映済み）
- 直前の公開ビルド: `v101`（Issue #7 席のゴースト対策。**実機QAは未実施のまま**）
...
- 現在の進行: GitHub Issue #23「すぐ対戦のマッチメイキング」(v102) を公開。**v101・v102とも実機QAが未実施のまま残っている。** 次は 段B 4人の席とロビー (v103) → 段C オンライン2vs2 (v104) → 段D 切断・CPU引き継ぎ #8 (v105)。
