...
- ブランチ: `master`
- 作業中ブランチ: `claude/katamon-handoff-avsydw`（v122 手番表示とテンポ。未公開）
- 現行ビルド: `v122`（手番が誰か分かるようにし、テンポを落ち着かせる。未公開／ルール変更なし）
- 直前の公開ビルド: `v121`（段D 切断・CPU引き継ぎ。PR #45。**ルール変更あり＝Console反映が要る**）
...
- 現在の進行: v121（段D 切断・CPU引き継ぎ）まで公開済み。その上に v122 として、実機報告のあった「開始カットイン中に撃たれる」「誰の手番か分からない」「テンポが速い」を直した（未公開・ルール変更なし）。指定の全テストは合計 **901件成功**。次は Issue #5（対戦部屋内の戦績表示）。
...
7. ~~**v121 段D / Issue #8 切断・CPU引き継ぎ**~~ — 公開済み（PR #45）。**ルール変更あり。**
   ...
   - Issue #8 の残りは「同一ラウンドへの復帰」。今回入れたのは引き継ぎだけなので、
     復帰の側はIssueに残る。
8. **v122 手番が誰か分かるようにし、テンポを落ち着かせる** — 実装済み・未公開。ルール変更なし。
