...
- ブランチ: `master`
- 現行ビルド: `v100`（Issue #20 / 段A：CPUだけの2vs2。ブランチ `claude/katamon-handoff-xn37i5`）
...
- 現在の進行: GitHub Issue #20「2vs2 段A（CPUだけの4人戦）」(v100) を実装完了。次は #7 席のゴースト対策 (v101) → マッチメイキング基盤 (v102) → 段B 4人の席とロビー (v103) → 段C オンライン2vs2 (v104) → 段D 切断・CPU引き継ぎ #8 (v105)。
...
- **Issue #20（2vs2 段A：CPUだけの4人戦）→ v100**

### これから（承認済みの順）

1. **v101 / Issue #7 席のゴースト対策**
   - 退出済みのゲスト・観戦席が残る問題。4人ぶんの席を扱う前に潰しておく。
2. **v102 マッチメイキング基盤**
