...
- ブランチ: `master`
- 現行ビルド: `v117`（キャラの見かけの大きさを揃える＋当たり判定の可視化。**未公開**／ルール変更なし）
- 直前の公開ビルド: `v116`（キャラ画像にキャッシュ版数を付ける）
...
- 現在の進行: 1台での2vs2は完走・再戦とも実機で確認済み。v109〜v117 でUI・起動演出・キャラ表示に手を入れた。残る未検証は**2台以上での受け取り側**。次は 段D 切断・CPU引き継ぎ #8 (v118)。
...
> ルールの変更なし。Console作業は不要。**遊びの数値は何も変えていない**（見た目だけ）。
