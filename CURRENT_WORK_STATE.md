...
- ブランチ: `master`
- 現行ビルド: `v101`（Issue #7 席のゴースト対策。master へマージ済み／Firebase Consoleルール反映済み）
- 直前の公開ビルド: `v100`（Issue #20 段A：CPUだけの2vs2。ユーザー実機確認済み）
...
- 現在の進行: GitHub Issue #7「席のゴースト対策」(v101) を公開。**実機QA待ち**（特に画面ロックからの復帰で追い出されないこと）。次は マッチメイキング基盤 (v102) → 段B 4人の席とロビー (v103) → 段C オンライン2vs2 (v104) → 段D 切断・CPU引き継ぎ #8 (v105)。
