...
- `database.rules.json` は **Console へ反映済み**（2026-08-04、ユーザー操作）。クライアントを公開してよい状態。
...
- 現在の進行: 段B（Issue #25）と 段C（Issue #26）の実装が終わり、**ルールもクライアントも4人ぶんへ広がっている**。ルールは反映済みで、残るのは**クライアントの公開（masterへのマージ）と実機QA**。**v101・v102・v104 とも実機QAが未実施のまま残っている。** 次は 段D 切断・CPU引き継ぎ #8 (v105)。
...
> ✅ **ルールは反映済み**（2026-08-04、ユーザー操作）。クライアントの公開はこれから。
...
3. ~~**v103 段B：4人の席とロビー**~~ ／ ~~**v104 段C：オンライン2vs2**~~ — 実装完了。**未公開。**
   - ルールは Console へ反映済み（2026-08-04）。残っているのは (1) クライアントの公開、(2) 4端末で実機QA。
