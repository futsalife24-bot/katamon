...
- ブランチ: `master`
- 現行ビルド: `v102`（Issue #23 すぐ対戦のマッチメイキング。ブランチ `claude/katamon-handoff-xn37i5`／**Consoleルール反映待ちでマージ前**）
- 公開済みビルド: `v101`（Issue #7 席のゴースト対策。ルール反映済み・実機QAは未実施）
...
- 現在の進行: GitHub Issue #23「すぐ対戦のマッチメイキング」(v102) を実装完了。**Consoleルール反映待ち。** v101の実機QAも未実施のまま残っている。次は 段B 4人の席とロビー (v103) → 段C オンライン2vs2 (v104) → 段D 切断・CPU引き継ぎ #8 (v105)。
...
> クライアントを公開すること。v102時点で**ルールは未反映**。
> ただし今回は v101 と違い、**順番を逆にしても入室が壊れることはない**（下記）。
...
2. ~~**v102 / Issue #23 マッチメイキング基盤**~~ — 実装完了・**Consoleルール反映待ち**。
3. **v103 段B：4人の席とロビー**
...
8. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
