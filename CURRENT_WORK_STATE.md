...
- ブランチ: `master`
- 作業中ブランチ: `claude/katamon-handoff-avsydw`（v127 チュートリアルの手直し。未公開）
- 現行ビルド: `v127`（チュートリアル最後の足場／VSカットの表示時間。未公開／ルール変更なし）
- 直前の公開ビルド: `v126`（チュートリアル「あそび方」。PR #51）
- その前: `v125`（1vs1の胴体の空きに情報／元素材の削除。PR #50）
- さらに前: `v121`（段D 切断・CPU引き継ぎ。PR #45。**ルール変更あり＝Console反映が要る**）
...
13. **v127 チュートリアル最後の足場／VSカットの表示時間** — 実装済み・未公開。
    - 足場が1発で崩れず終われなかった不具合の修正と、VSカットの表示時間を2倍に。
14. Issue #5 対戦部屋内の戦績表示
...
16. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
