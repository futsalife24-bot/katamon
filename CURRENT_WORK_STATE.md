...
- ブランチ: `master`
- 現行ビルド: `v105`（v104の実機で見つかった3件の修正。master へマージ済み／ルール変更なし）
...
- 現在の進行: v104（段B＋段C）を公開後、実機で**対戦方式を2vs2に変えられない**不具合が見つかり v105 で修正・公開（PR #28）。**2vs2の実機QAはこれから。** 次は 段D 切断・CPU引き継ぎ #8 (v106)。
