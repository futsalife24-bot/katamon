...
- ブランチ: `master`
- 作業中ブランチ: `claude/katamon-handoff-avsydw`（v128 撃破済みへのダメージ表示／砲弾の絵。未公開）
- 現行ビルド: `v128`（撃破済みキャラへのダメージ表示／起動演出の砲弾の絵。未公開／ルール変更なし）
- ひとつ前: `v127`（チュートリアル最後の足場／VSカットの表示時間。PR #52）
- その前: `v126`（チュートリアル「あそび方」。PR #51）
- さらに前: `v125`（1vs1の胴体の空きに情報／元素材の削除。PR #50）
...
- 現在の進行: v127 まで公開済み。その上に v128 として、実機確認で出た「撃破済みのキャラにダメージ数字が出る」を直し、起動演出の砲弾をユーザー提供の絵へ差し替えた（未公開・ルール変更なし）。指定の全テストは合計 **1033件成功**。次は Issue #5（対戦部屋内の戦績表示）。
...
14. **v128 撃破済みへのダメージ表示／起動演出の砲弾の絵** — 実装済み・未公開。
    - 薄く描かれているキャラにダメージ数字が出ていた不具合の修正と、砲弾の素材差し替え。
15. Issue #5 対戦部屋内の戦績表示
...
17. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
