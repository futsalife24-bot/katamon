...
- 現在の進行: GitHub Issue #2「v91 実機オンライン対戦のリリースゲート」。初回砲撃401を再現し、Firebase Consoleへの現行ルール反映待ち。
...
- ブロッカー: クイック対戦と固有合言葉部屋の両方で、ホストの初回砲撃だけFirebase書き込みが401となり、ホストが中断画面、ゲストが1ターン目待機のままになる。
- 有力原因: v91コミット`14ea290`で`fire.useJump`とローカル`database.rules.json`の検証を追加したが、公開中Firebase Consoleルールが旧版のままと推定される。ロビー・開始パケットは直前まで成功しており、実際の認証期限切れよりルール拒否と整合する。
- 次の確認: 現行`database.rules.json`をFirebase Consoleへ反映後、初回砲撃の再試験と独立2端末QAを行う。
- 未確認: 実機オンライン対戦の全経路、闘技場の壁へ跳躍した際の最終的な操作感。
