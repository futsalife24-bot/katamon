...
- ブランチ: `master`
- 作業中ブランチ: `fix/v120-lobby-stale-seat`（幽霊席の削除可否表示。v119の上に積んだ未公開ブランチ）
- 現行ビルド: `v118`（直撃の円をキャラの体へ上げる。master へマージ済み／ルール変更なし）
- 直前の公開ビルド: `v117`（見かけの大きさを揃える＋当たり判定の可視化）
...
- 現在の進行: v119 のVSカットインと、ユーザーQAで見つかったロビーの幽霊席修正をv120として実装済み。ローカルHTTPのPCタブ2つでは、2vs2の開始・双方の操作・HP同期・CPU手番・決着まで確認した。次はv119/v120をレビューし、スマホ＋PCタブ2つで幽霊席の修正を再確認する。段D 切断・CPU引き継ぎ #8 は v121 へ繰り下げる（2026-08-04）。
...
> ⏳ **未公開**（作業ブランチ `fix/v120-lobby-stale-seat`）。`database.rules.json` の変更なし。
...
> ⏳ **未公開**（作業ブランチ `feat/v119-vs-cutin`）。ルールの変更なし。ローカル実ブラウザ確認済み。
...
4. **v119 バトル開始時のVSカットイン** — 実装済み・未公開。ローカル実ブラウザで1vs1確認済み。
5. **v120 ロビーの幽霊席表示と削除可否を一致させる** — 実装済み・未公開。
6. **スマホ＋タブ2つでオンライン2vs2を通す**
...
7. **v121 段D / Issue #8 切断・CPU引き継ぎ**
   - 落ちた人の席をCPUが引き継ぎ、試合を最後まで終わらせる（決定2）。
8. Issue #5 対戦部屋内の戦績表示
...
10. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
