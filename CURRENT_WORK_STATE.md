...
## カスタムステージ固定ボタンの表示修正（公開前）

- 作業ブランチ: `fix/custom-stage-launcher-visibility`
- 修正: 画面下部の`カスタムステージ`固定ボタンは、起動前の`TAP TO START`とタイトルでは非表示にし、`演習`の設定画面だけで表示する。
- キャッシュ: ゲーム本体のビルド表示とService Workerを`v138-stage-studio-launcher`へ更新した。
- オンライン: 現行MVPは同一端末の演習専用。4項目の照合データ構造はあるが、安全なステージ配布、参加者全員の一致確認、開始ロックは未実装のためオンライン対戦では使用不可。
- 自動確認: Stage基盤47/47件成功。JSONの作成・出力・ゲームインポート・バトル開始はiPhone相当WebKitとAndroid相当Chromiumで成功。ZIP試験は失敗記録なしだが、Windows上のPlaywright終了処理が固まり完了コードを取得できていない。
- 実機確認: 未実施。
