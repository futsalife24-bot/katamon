...
22. **v139 指定された爆発音へ差し替え、起動砲弾にも共用** — PR #66、承認待ち。
   ...
   - やってはいけないこと: 必殺技・跳躍・電磁波・花火の拡散弾へこの音を広げない。起動砲弾は飛んだ時ではなく壁へ当たった時にだけ鳴らす。必殺カットイン音の変更をこの版へ混ぜない。外部URLをゲーム中に直接再生しない。音源の中身を替える時はファイルのハッシュとURLの `?v=` を一緒に更新する。通常弾を `0.4` 超、起動着弾を `0.3` 超へ戻さない。音の読み込み結果や端末時刻を命中・ダメージ・通信などの対戦判定へ使わない。
   - 新規検査は、旧実装で `regressiontest p1` が `258 passed, 1 failed` になることを先に確認。音量検査も大きい値へ戻した壊れた実装で `stage3test` が `398/399 passed` になることを確認した。修正後は `seattest 20件×2席 + regressiontest 259件×2席 + resulttest 96件 + stage3test 399件 + lobbysimtest 7件 + loopbacktest 103件`、合計 **1163件すべて成功**。中継数は `38 / 64 / 83 / 61 / 48` のまま。
   - ローカルHTTPと実ブラウザで、v139表示、タップ砲弾が壁へ着弾してタイトルへ移ること、指定音源が `assets/normal-impact-explosion.mp3?v=2` から読み込まれること、CPU戦で通常弾が着弾して手番が進むことを確認。ブラウザのエラー・警告は0件。
   - `BUILD_ID` と `CACHE_VERSION` は `v139` に揃えた。`database.rules.json` は変更なし。Firebase Console作業なし。
23. **v140 必殺カットイン音をスタイリッシュな音へ変更** — 未着手。
   - v139と同じく、必要ならPixabayから音源を取得し、配布元URLとライセンス確認日を記録する。
24. Issue #5 対戦部屋内の戦績表示
...
26. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
