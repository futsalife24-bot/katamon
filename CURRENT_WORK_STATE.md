...
- ブランチ: `master`
- 作業中ブランチ: `feat/v133-wind-forecast`（PR #59。メロニキ承認前にマージしない）
- 現行ビルド: `v132`（タイトル画面の静止部分を1枚に焼く。PR #58。公開済み）
- 次の候補: `v133`（次の風を1区間先まで予報。作業中・未公開）
- ひとつ前: `v131`（キャンバスを画面の実画素ぴったりに。PR #56）
- その前: `v130`（起動時に読む絵をWebPへ。PR #55）
- さらに前: `v129`（空と地形を1枚に焼いて貼る。PR #54）
...
- 現在の進行: **v132「タイトル画面を焼く」まで公開済み。v133「次の風予報」は PR #59 で承認待ち・未公開。**
  v133候補は追加27件を含む合計 **1125件成功**。メロニキ承認待ちで止める。
  v133の次は、繰り上げ前の順へ戻り Issue #5（対戦部屋内の戦績表示）→ Issue #6（ランキング改修）→ Issue #4（オンラインHPバーのずれ）。
...
> ⏳ **未公開・PR #59 承認待ち**（作業ブランチ `feat/v133-wind-forecast`）。ルール変更・Console作業は不要。
...
16. **v133 次の風予報** — メロニキの指示で最優先へ繰り上げ。PR #59 承認待ち・未公開。
   - 1区間先を常時表示し、予報値を次の切替でそのまま使う。
   - オンライン・観戦・中断再開へ同じ予報を同期。全1125件成功。
17. Issue #5 対戦部屋内の戦績表示
...
19. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
