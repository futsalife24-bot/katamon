...
- Firebase Console: `database.rules.json`は変更していないため作業不要。本番ランキングへ反映するには、既存のv179変更と今回のv182変更を含む最新`gas/ranking.gs`をApps Scriptへ貼り付け、新バージョンとして再デプロイする必要がある。
...
- GAS反映: ローカルの`gas/ranking.gs`と手順書は更新済み。メロニキがApps Scriptへ全量貼り付けて新バージョンをデプロイするまで、本番APIへの反映確認は未完了。URLは変わらないため、ゲーム側のURL変更は不要。
