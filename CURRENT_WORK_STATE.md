...
- ブランチ: `master`
- 現行ビルド: `v110`（起動演出:砦の壁を撃ち抜いてタイトルへ。**未公開**／ルール変更なし）
...
- 現在の進行: 1台での2vs2は完走・再戦とも実機で確認済み。v109〜v110 でUIと起動演出に手を入れた。残る未検証は**2台以上での受け取り側**。次は 段D 切断・CPU引き継ぎ #8 (v111)。
...
> ルールの変更なし。Console作業は不要。ユーザー発案（2026-08-04）。
