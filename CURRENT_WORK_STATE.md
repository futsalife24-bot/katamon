...
- ブランチ: `master`
- 現行ビルド: `v115`（ニセンモノのキャラ画像を差し替え。**未公開**／ルール変更なし）
- 直前の公開ビルド: `v114`（壁の割れ方を不揃いにする）
...
- 現在の進行: 1台での2vs2は完走・再戦とも実機で確認済み。v109〜v115 でUI・起動演出・キャラ画像に手を入れた。残る未検証は**2台以上での受け取り側**。次は 段D 切断・CPU引き継ぎ #8 (v116)。
...
> ルールの変更なし。Console作業は不要。画像はユーザー生成。
