...
- ブランチ: `master`
- 作業中ブランチ: `claude/katamon-handoff-avsydw`（v121 段D。未公開）
- 現行ビルド: `v121`（段D 切断・CPU引き継ぎ。**未公開／ルール変更あり・Console反映が必要**）
...
- 現在の進行: v119・v120 は公開済み。v121 として 段D / Issue #8「切断・CPU引き継ぎ」を実装した（未公開）。指定の全テストは合計 **874件成功**。**公開前に `database.rules.json` を Firebase Console へ反映すること。** 次は Issue #5（対戦部屋内の戦績表示）。
...
7. **v121 段D / Issue #8 切断・CPU引き継ぎ** — 実装済み・未公開。
