...
- ブランチ: `master`
- 現行ビルド: `v106`（CPUだけの相手で35秒で切られる不具合の修正。**未公開**／ルール変更なし）
...
- 現在の進行: v105 で2vs2が実際に遊べることを実機で確認（4手番・4体ぶんのHUD・ダメージとも正常）。ただし**CPUだけの相手だと35秒で切断判定に落ちる**ため v106 で修正。次は 段D 切断・CPU引き継ぎ #8 (v107)。
