...
- ブランチ: `master`
- 現行ビルド: `v118`（直撃の円をキャラの体へ上げる。**未公開**／ルール変更なし）
...
- 現在の進行: 1台での2vs2は完走・再戦とも実機で確認済み。v109〜v118 でUI・起動演出・キャラ表示に手を入れた。残る未検証は**2台以上での受け取り側**。次は 段D 切断・CPU引き継ぎ #8 (v119)。
...
> ルールの変更なし。Console作業は不要。**爆風のダメージ計算は一切変えていない。**
...
- 自動テスト合計 **807件すべて成功**（regressiontest へ 3件追加）。
...
- 自動テスト合計 **804件すべて成功**（regressiontest へ 3件追加、死神の固定値を更新）。
