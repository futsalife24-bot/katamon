...
- `0.4.4` で「画像、向きと基準点、生成、キャラ、GitHub」の5ステップへ縮小した。能力・必殺技・プリセット選択・モーション調整スライダー、目の推測・合成は日常UIから外した。
...
- 型検査と87件の単体・結合テスト、クライアント／サーバービルド、提供画像を使うAndroid 13相当E2E、既存ゲーム1,160件 + Stage基盤37件（合計1,197件）が成功。被弾専用画像の端末内保存・復旧、通常画像フォールバック、向き別112°回転、反転姿勢での後方着地・低いバウンド・接地待機・元位置復帰、通常姿勢へ固定した当たり判定候補、モーション選択時の先頭再生、旧下書きからの目データ除去も自動または実表示で確認した。Stage StudioのAndroid相当4件も全アサーション成功したが、Windows上でPlaywright終了処理が残る事象は別途確認が必要。提供画像はコミット・外部送信しない。物理Android実機のPWAインストール、OS共有ターゲット、実GitHub認証は未確認。
