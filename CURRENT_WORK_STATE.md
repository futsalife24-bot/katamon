...
- `0.4.3` で「画像、向きと基準点、生成、キャラ、GitHub」の5ステップへ縮小した。能力・必殺技・プリセット選択・モーション調整スライダー、目の推測・合成は日常UIから外した。
- 通常画像から前進、後退、単発砲撃、被弾、着地の5クリップを固定設定で一括生成する。被弾だけ任意の専用画像を追加でき、未選択なら通常画像を使う。右向きは反時計回り112°、左向きは鏡方向へ後方へ吹き飛びながら浮上回転する。反転姿勢のまま着地して低くバウンドし、後方の接地位置で数フレーム待ってから元位置へ戻る。当たり判定候補は通常姿勢の位置へ固定する。
