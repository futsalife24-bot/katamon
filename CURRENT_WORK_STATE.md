...
- ブランチ: `master`
- 作業中ブランチ: `claude/katamon-handoff-avsydw`（v130 画像の圧縮。未公開）
- 現行ビルド: `v130`（起動時に読む絵をWebPへ。未公開／ルール変更なし）
- ひとつ前: `v129`（空と地形を1枚に焼いて貼る。PR #54）
- その前: `v128`（撃破済みへのダメージ表示／起動演出の砲弾の絵。PR #53）
- さらに前: `v127`（チュートリアル最後の足場／VSカットの表示時間。PR #52）
...
- 現在の進行: v129 まで公開済み。その上に v130 として、起動時に読む絵をWebPへ移した（未公開・ルール変更なし）。指定の全テストは合計 **1062件成功**。軽量化は v129=描き方 → v130=画像 → 次=解像度の上限 の順。**加えて、タイトル画面が重い原因が `drawTitleScreen`（1コマ52ミリ秒、タイトルの半分以上）だと分かったので、4つ目として足す。** その後 Issue #5（対戦部屋内の戦績表示）。
...
15. **軽量化（4版に分ける）** — ユーザー要望・2026-08-05。実機で「動作が重い」。
    ...
    - v130 画像の圧縮（起動時に読む絵をWebPへ。3.83MB→0.74MB）— 実装済み・未公開。
    - 次: 解像度の上限（dprを2で頭打ち）。実測2.5倍速。僅かに眠くなるので実機で判断する。
    - その次: **タイトル画面を焼く。`drawTitleScreen` が1コマ52ミリ秒で、タイトルの半分以上。**
      実機で「まだタイトルから重い」と言われている（2026-08-05）。
16. Issue #5 対戦部屋内の戦績表示
...
18. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
