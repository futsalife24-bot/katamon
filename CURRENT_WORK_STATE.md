...
- 順番: 先に予定していた必殺カットイン音の差し替えは、今回の鋼鉄地形を優先したため **v149** へ繰り下げた。`database.rules.json`は変更なしで、Firebase Console作業は不要。
...
26. ~~**v148 壊れない鋼鉄地形**~~ — PR作成前。詳細は冒頭のv148節を正本とする。
27. **v149 必殺カットイン音をスタイリッシュな音へ変更** — 未着手。
   - v144と同じく、必要ならPixabayから音源を取得し、配布元URLとライセンス確認日を記録する。
28. **v150 標準／大型ステージの選択式** — 未着手。
   - 大型は高さを活かす専用の出撃位置・カメラ範囲・Stage Studio生成／検査をまとめて扱い、通常サイズと選べるようにする。
29. Issue #5 対戦部屋内の戦績表示
...
31. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
