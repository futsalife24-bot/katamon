...
- `0.2.0` で日常UIをモーション生成へ特化した。画面は画像、切り抜き、部位候補、動作、確認、出力の6ステップ。
- 標準は高品質512px。高解像度元画像から一度だけ正規化し、再生時Canvasを端末倍率へ合わせる。Worker未対応時だけ256px軽量代替を明示して使う。
- 待機、移動、砲撃、被弾の4動作に各3プリセット。部位候補は透明輪郭を5領域へ端末内分割し、動作中心または接地点として使う。意味的な部位認識と個別変形は未実装。
- 常時表示のrange sliderをやめ、閉じた微調整と大きな増減ボタンに変更。画像Canvas上からも縦スクロールできる。
- 出力はスプライトPNG、メタデータJSON、モーションプロファイルJSON、および3点をまとめたZIP。
- 型検査、80件の単体・結合テスト、ビルド、Android 13相当（412×915、touch、DPR 2.625）のE2Eが成功。物理Android実機のPWAインストールと共有ターゲットは未確認。
