...
- ブランチ: `master`
- 作業中ブランチ: `claude/katamon-handoff-avsydw`（v125 1vs1の情報。未公開）
- 現行ビルド: `v125`（1vs1の胴体の空きに情報／元素材の削除。未公開／ルール変更なし）
- 直前の公開ビルド: `v124`（VSカットインの顔の位置と砲弾の動き。PR #49）
- その前: `v121`（段D 切断・CPU引き継ぎ。PR #45。**ルール変更あり＝Console反映が要る**）
...
- 現在の進行: v123 まで公開済み。その上に v124 として、実機確認で出た「顔が窓に合っていない」「砲弾の向きが不自然」「最後に撃ち抜けてほしい」の3点を直した（未公開・ルール変更なし）。指定の全テストは合計 **939件成功**。次は Issue #5（対戦部屋内の戦績表示）。
...
10. **v124 VSカットインの顔の位置と砲弾の動き** — 実装済み・未公開。ルール変更なし。
    - 顔の切り出しを6体ぶん直し、砲弾が軸に沿って飛び、最後に撃ち抜けるようにした
      （実機指摘・2026-08-05）。
11. **v125 1vs1の胴体の空きに情報を入れる** — 実装済み・未公開。ルール変更なし。
    - 役割・最大HP・必殺技の3行。あわせて切り出し済みの元素材を削除（実機指摘・2026-08-05）。
12. Issue #5 対戦部屋内の戦績表示
...
14. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
