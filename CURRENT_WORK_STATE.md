...
- ブランチ: `master`
- 現行ビルド: `v104`（Issue #25 段B ＋ Issue #26 段C。**未公開。ブランチ `claude/continue-from-here-4nuuh0` にある**）
- 直前の公開ビルド: `v102`（Issue #23 すぐ対戦のマッチメイキング。master へマージ済み／Consoleルール反映済み）
- **v104 を公開する前に、`database.rules.json` を Firebase Console へ反映すること。** 反映前に公開すると、s1/s2 の人が準備完了すら送れない。
...
- 現在の進行: 段B（Issue #25）と 段C（Issue #26）の実装が終わり、**ルールもクライアントも4人ぶんへ広がっている**。残るのは Console 反映と実機QA。**v101・v102・v104 とも実機QAが未実施のまま残っている。** 次は 段D 切断・CPU引き継ぎ #8 (v105)。
...
> ⚠️ **未公開。`database.rules.json` の Console 反映がまだ。** 反映してから公開すること。
...
- 自動テスト合計 **754件すべて成功**（stage3test へ 26件追加）。
...
- **4端末での通し。** 自動テストは受信経路と判定関数までで、4台が同時につながった状態は
  一度も通していない。`loopbacktest.js` は2人ぶん・v2プロトコルなので、ここは埋まっていない。
- 空席のCPUが撃つところ（ホスト以外の画面で、歩き・弾道・ダメージが一致すること）。
- 2人だけで始めた2vs2（CPU2体）と、3人＋CPU1体。
- 陣営全滅での決着と、4人ぶんの再戦。
- 4体ぶんのHPバーとターンバーの見え方。
...
3. ~~**v103 段B：4人の席とロビー**~~ ／ ~~**v104 段C：オンライン2vs2**~~ — 実装完了。**未公開。**
   - 残っているのは2つだけ: (1) `database.rules.json` を Firebase Console へ反映する、(2) 4端末で実機QA。
   - Console反映を先に済ませること。反映前に公開すると s1/s2 の人が準備完了すら送れない。
4. **v105 段D / Issue #8 切断・CPU引き継ぎ**
   - 落ちた人の席をCPUが引き継ぎ、試合を最後まで終わらせる（決定2）。
5. Issue #5 対戦部屋内の戦績表示
...
7. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
