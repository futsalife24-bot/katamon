...
- ブランチ: `master`
- 現行ビルド: `v101`（Issue #7 席のゴースト対策。ブランチ `claude/katamon-handoff-xn37i5`／**Firebase Consoleルール未反映のためマージ前**）
- 公開済みビルド: `v100`（PR #21 で master へマージ済み・ユーザー実機確認済み）
...
- 現在の進行: GitHub Issue #7「席のゴースト対策」(v101) を実装完了。**`database.rules.json` のConsole反映待ちで、反映まではマージしない。** 次は マッチメイキング基盤 (v102) → 段B 4人の席とロビー (v103) → 段C オンライン2vs2 (v104) → 段D 切断・CPU引き継ぎ #8 (v105)。
...
> ⚠️ **公開手順に順番がある。** `database.rules.json` を **先に** Firebase Console へ反映してから、
> クライアント（master／GitHub Pages）を公開すること。逆にすると**誰も入室できなくなる。**
> 理由は下の「公開の順番」に書いた。v101時点で**ルールは未反映**。
...
オンライン2vs2（段C）の土台となる部分を先に固めた。Issue #4（HP表示の作り替え）もここへ吸収した。
...
- Issue #4（HP表示の作り替え）→ **v100 の 2vs2 HUD へ吸収**
- **Issue #20（2vs2 段A：CPUだけの4人戦）→ v100（公開済み）**

### これから（承認済みの順）

1. **v101 / Issue #7 席のゴースト対策** — 実装完了・**Consoleルール反映待ち**
   - 反映してからマージすること（順番を逆にすると入室できなくなる。上記v101の節を参照）。
2. **v102 マッチメイキング基盤**
   - 公開された部屋一覧ノードの新設。**`database.rules.json` の変更とFirebase Console反映が必要**。
   - 現状「クイック対戦は世界に1部屋（`KATAMN22`固定）」を、条件ごとの複数部屋へ広げる。
3. **v103 段B：4人の席とロビー**
   - 既存の観戦席 `s1`/`s2` を対戦者席へ転用する（決定8）。
4. **v104 段C：オンライン2vs2**
   - スナップショットは v100 で `matchFormat` を持つよう拡張済み。形式を持たない相手は1vs1として読む。
5. **v105 段D / Issue #8 切断・CPU引き継ぎ**
   - 落ちた人の席をCPUが引き継ぎ、試合を最後まで終わらせる（決定2）。
6. Issue #5 対戦部屋内の戦績表示
