...
## v164 部屋内のモンスター選択にキャラ画像を表示（PR #107で検査中）

- 作業ブランチ: `feat/v164-lobby-monster-images`。公開済みv163（PR #106）から、メロニキの「部屋内のモンスター選択でもキャラ画像を見たい」という要望だけを扱い、PR #107を作成した。
...
- 表示確認: ローカルHTTPでv164の`index.html`が`200`で配信されることを確認した。このセッションで接続できる実ブラウザは0台だったため、直接の目視確認は未実施。PRの`mobile-e2e`でスマホ幅の実ブラウザ検査を通し、公開後はメロニキの実機で最終確認する。
...
42. **v164 部屋内のモンスター選択にキャラ画像を表示** — PR #107で検査中。
   - 自分の選択欄へ、元の縦横比の全身画像と名前を並べる。相手の確定前キャラは公開しない。
43. Issue #6 ランキング改修
   - 同一プレイヤー3件まで。
   - GASの `character` 列が再デプロイ済みか確認する。
44. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
