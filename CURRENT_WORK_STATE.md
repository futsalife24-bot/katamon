...
- ブランチ: `master`
- 現行ビルド: `v114`（壁の割れ方を不揃いにする。**未公開**／ルール変更なし）
- 直前の公開ビルド: `v113`（壁を生成画像へ。**割れ方が賽の目に見える問題あり → v114で修正**）
- 直前の公開ビルド: `v112`（起動演出の作り込み）
- 直前の公開ビルド: `v111`（タイトルが震え続ける不具合の修正）
- 直前の公開ビルド: `v110`（起動演出。**タイトルが震え続ける不具合あり → v111で修正**）
- 直前の公開ビルド: `v109`（部屋とタイトルのボタンの見分けを付ける）
- 直前の公開ビルド: `v108`（401でやり直さない不具合の修正＋「CPUで始める」廃止）
...
- 現在の進行: 1台での2vs2は完走・再戦とも実機で確認済み。v109〜v114 でUIと起動演出に手を入れた。残る未検証は**2台以上での受け取り側**。次は 段D 切断・CPU引き継ぎ #8 (v115)。
...
> ルールの変更なし。Console作業は不要。v113への指摘。
