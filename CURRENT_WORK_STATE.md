...
- 作業ブランチは `feat/content-studio-mvp`。ゲーム本体の `master` へ直接反映せず、完成前に `origin/master` の `v138-stage-studio-mvp` を統合した。
- `0.5.0` で「画像、向きと基準点、生成、キャラ、GitHub」の5ステップを維持しつつ、5モーションそれぞれへ48dp以上の「控えめ・標準・激しめ」を追加した。大量の調整スライダー、能力・必殺技、目の推測・合成は日常UIへ戻していない。
- ダッシュボードから既存16体を1体ずつ選び、既存アセットを読み込んでモーションだけを後付けできる。表示名、能力、技、静止画像は読み取り専用で保持し、`legacyTargetId` が一致する正規レコードだけを許可する。互換カタログは既存手書き定義へモーション参照だけを添付し、未更新キャラは従来の静止画像へフォールバックする。原則1体1PRで、移行済み体数を画面に表示する。ゲーム本体で状態に応じてスプライトを再生する処理は次段階として未実装。
- 通常画像から前進、後退、単発砲撃、被弾、着地の5クリップを固定設定で一括生成する。被弾だけ任意の専用画像を追加でき、未選択なら通常画像を使う。右向きは反時計回り112°、左向きは鏡方向へ浮上回転し、標準512px出力では約半身分に当たる128px後方へ吹き飛ぶ。軽量出力では同じ比率へ自動調整する。反転姿勢のまま着地して低くバウンドし、後方の接地位置で数フレーム待ってから元位置へ戻る。当たり判定候補は通常姿勢の位置へ固定する。
- 接地点と砲口を通常の端末内画像処理で推測し、ズレた位置は画像タップで修正する。生成AI、LLM、外部画像処理APIは使わない。
...
- 型検査と91件の単体・結合テスト、クライアント／サーバービルド、Android 13相当E2E 2件、既存ゲームとStage基盤の全テストが成功。新規フローに加え、モーション別3段階、既存キャラ画像の読み込み、ID入力のロック、旧下書きからの移行、別キャラへ向けた不正な既存更新の拒否を確認した。提供画像はコミット・外部送信しない。物理Android実機のPWAインストール、OS共有ターゲット、実GitHub認証、ゲーム本体での5モーション再生は未確認・未実装。
