...
- ブランチ: `master`
- 作業中ブランチ: `feat/v136-emp-hit-feedback`（目玉の電磁波の命中表示と移動不可エフェクト。PR #62、レビュー待ち）
- 現行ビルド: `v135`（全16キャラの通常弾を共通化。PR #61。公開済み）
- 次に公開するビルド: `v136`（目玉の電磁波の命中表示と移動不可エフェクト。実装・検証済み）
...
- 現在の進行: **v136「目玉の電磁波の命中表示と移動不可エフェクト」を実装・検証済み。PR #62、レビュー待ち。**
  v136は追加8件（2席で新規4件ずつ）を含む合計 **1147件成功**。
  v136の公開後は、各キャラの必殺技調整の相談を続ける。差し込み前の残タスク順は Issue #5（対戦部屋内の戦績表示）→ Issue #6（ランキング改修）→ Issue #4（オンラインHPバーのずれ）。
...
19. **v136 目玉の電磁波の命中表示と移動不可エフェクト** — メロニキの指示で差し込み。PR #62、レビュー待ち。
   - 電磁波の着弾結果カットインは、相手チームのキャラへ実際に命中した時だけ出す。空振りと味方だけへの命中では成功表示を出さない。
   - 移動不可のキャラには、残り手数つきの `移動不可 N` 表示と水色・紫の電気エフェクトを継続表示する。既存の `moveLockTurns` を表示に使い、通信する状態は増やしていない。
   - 理由: 以前は `emitEmp` が命中の有無に関係なく着弾後の成功カットインを出しており、空振りでも移動不可になったように見えていた。状態表示もなく、誰が何手動けないか分かりにくかった。
   - やってはいけないこと: 発射時の「必殺: 電磁波!!」は消さない。電磁波のダメージ・味方への当たり方・移動不可の手数は変えない。表示用の時刻や端末ごとに変わる値を対戦判定へ使わない。表示は必ず `moveLockTurns` から作る。
   - 新規4件は、修正前の状態で `249 passed, 4 failed` になることを先に確認し、修正後は両席とも253件成功。全体は1147件成功。中継数は `38 / 64 / 83 / 61 / 48` のまま。
   - ローカルHTTPと実ブラウザで、空振り時に成功表示が出ないこと、相手命中時だけ `電磁波命中: 移動不可 2手` が出ること、対象の頭上表示と電気エフェクトがHP表示やキャラに不自然に重ならないことを確認。ブラウザのエラー・警告は0件。
   - `database.rules.json` は変更なし。Firebase Console作業なし。
20. Issue #5 対戦部屋内の戦績表示
...
22. **Issue #4 オンラインHPバーの瞬間的なずれ** — 未着手。
