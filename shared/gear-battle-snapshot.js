(function initKatamonGearBattleSnapshot(root, factory) {
  ...
  function createBattleGearSnapshot({ resolvedLoadout, baseHp, baseFuel }) {
    ...
    return validateBattleGearSnapshot({ version: VERSION, characterId: resolvedLoadout.characterId, presetId: resolvedLoadout.presetId, slots: slotViews, derivedStats: clone(derivedStats), activeSets: clone(derivedStats.activeSets), initialRuntimeState: clone(combat().createRuntimeEffectsState()) }, { baseHp, baseFuel });
  }
  ...
  function stableSerialize(snapshot) { return domain().stableStringify(validateBattleGearSnapshot(snapshot, { baseHp: snapshot.derivedStats.baseHp, baseFuel: snapshot.derivedStats.baseFuel })); }
  function validateBattleGearSnapshot(snapshot, { baseHp, baseFuel }) {
    ...
    if (typeof snapshot.characterId !== 'string' || !snapshot.characterId || !presets().PRESET_IDS.includes(snapshot.presetId)) fail('INVALID_GEAR_BATTLE_SNAPSHOT'); exact(snapshot.slots, slots(), 'INVALID_GEAR_BATTLE_SNAPSHOT'); const seen = new Set(); const full = [];
