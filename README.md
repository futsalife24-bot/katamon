...
タイトルのGARAGEまたはGear Workbenchから、JST 0時更新・1日1射の専用チャレンジへ入れる。月曜から土曜は砲身、装甲、コア、動力、照準、補機の順で対象部位が替わり、日曜だけ6部位から選択できる。命中時は対象部位Gear 1個、外れ時も粉末3個を既存の未受取報酬として保存する。

入場、照準、退出では当日分を消費しない。FIREを端末へ保存・読戻しできた時だけ消費し、再読込時は再発射せず保存済み報酬を復旧する。完全オフライン機能のため、端末時計を未来へ進める操作やlocalStorage全消去までを完全には防げない。
...
曜日ダンジョンのJST境界、一回性、保存復旧、報酬とUI契約だけを実行する。
