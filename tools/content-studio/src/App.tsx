    ...
    <section className="step-panel motion-batch-step" data-testid="step-motion">
      ...
      <div className="motion-batch-list" aria-label="生成するモーション">
        {MOTION_CLIP_IDS.map((clipId, index) => <article key={clipId} className={studio.motions[clipId] ? 'is-complete' : ''}>
          <span>{index + 1}</span><div><b>{MOTION_CLIP_LABELS[clipId]}</b><small>{clipId === 'move-forward' ? '向きを保ったその場前進' : clipId === 'move-backward' ? '向きを保ったまま後ずさり' : clipId === 'fire' ? '1発だけの反動' : clipId === 'hit' ? `${draft.hitImageInfo ? '専用画像で' : ''}後方へ反転着地・低くバウンド・遅れて復帰` : '落下から接地して静止'}</small></div><strong>{studio.motions[clipId] ? '✓' : '—'}</strong>
        </article>)}
      </div>
      {generatedCount > 0 ? <>
