...
export function isAllowedGeneratedPath(path: string): boolean {
  ...
  return (
    ...
    /^generated\/content-studio-(?:catalog\.js|manifest\.json|registration\.json)$/u.test(path) ||
    /^assets\/content-studio\/[a-z][a-z0-9-]{0,23}\/[a-f0-9]{12}\/(?:source\.(?:png|jpe?g|webp)|character\.(?:png|webp)|icon\.png|thumbnail\.webp|idle\.(?:png|json)|(?:move-forward|move-backward|fire|hit|land)\.(?:png|json)|preview\.png|edit-(?:source|hit)\.png)$/u.test(path)
  );
}
