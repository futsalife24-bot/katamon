...
Content Studio は、対象ゲーム用のキャラクターモーションを Android スマートフォンで生成し、安全なPRとして登録する縦画面 PWA です。画像の取り込み、切り抜き、向き・接地点・砲口・目の確認、5モーション生成、最小キャラクター情報、GitHub反映を1つの下書きで管理します。

現行の `0.3.0` は「画像 → 向きと目 → 生成 → キャラ → GitHub」の5ステップです。生成する動きは、前進、後退、単発砲撃、被弾、着地だけに固定し、日常画面からプリセット選択や大量の調整項目を外しました。通常技は対象ゲームの標準弾、必殺技は「未設定（ボタン無効）」で登録し、後からゲーム側で設定します。
...
2. 「画像」でファイルまたはカメラを選び、透過状態と背景警告を確認します。同じ画面で自動背景除去、自動トリム、ブラシ、位置・倍率を必要な分だけ調整します。
3. 「向きと目」で元画像の向きを左・右から選び、「位置を自動検出」を押します。接地点、砲口、最大2つの目が推測されます。ズレた印を選び、画像をタップして直します。目の×印は大きな `−` / `＋` で調整できます。
...
画像・位置調整Canvasは `touch-action: pan-y` で縦スクロールを妨げません。入力画像は展開前に最大辺1600pxへ安全に縮小し、高画質512pxのモーション元へ一度だけ正規化します。プレビューCanvasは端末倍率へ合わせるため、再生時だけ急に粗くなりません。Web Workerが使えない端末だけ256pxの軽量代替へ切り替え、画面に明示します。単発砲撃、被弾、着地のゲーム用メタデータは非ループで、Studio上では動きを確認しやすいよう繰り返し表示します。
...
E2E は Chromium の Android 13 相当 User-Agent、`412 x 915`、タッチ、端末倍率 `2.625`、縦画面で、画像取り込み、画像上からのスクロール、切り抜き、向き・目の補正、5モーションの高品質512px生成、生成ボタンの画面内固定、ZIP保存、モックPR、CI成功後マージ、再起動後の下書き、オフライン復帰を確認します。初回は `npx playwright install chromium` でブラウザを用意してください。
...
- 位置検出は透明輪郭と局所コントラストを使う決定的な端末内推測です。接地点、砲口、目の初期位置を補助しますが、意味を理解するAI認識ではないため、ズレた場合は画像タップで直してください。耳、翼、武器、尻尾等の個別変形は未実装です。
