...
現行の `0.4.2` は「画像 → 向きと基準点 → 生成 → キャラ → GitHub」の5ステップです。生成する動きは、前進、後退、単発砲撃、被弾、着地だけに固定し、日常画面からプリセット選択や大量の調整項目を外しました。通常技は対象ゲームの標準弾、必殺技は「未設定（ボタン無効）」で登録し、後からゲーム側で設定します。
