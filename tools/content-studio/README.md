...
Content Studio は、対象ゲーム用のキャラクターモーションを Android スマートフォンで生成する縦画面 PWA です。画像の取り込み、切り抜き、部位候補の検出、動作プリセット、微調整、プレビュー、スプライト一式の出力までを1つの下書きで管理します。

現行の `0.2.0` は日常操作をモーション生成へ特化しています。キャラクター情報、技、GitHub反映の基盤コードは段階移行用に残していますが、通常画面には出しません。キャラクター設定と必殺技設定は生成したモーションを確認した後の別工程です。
...
1. ダッシュボードで「モーションを新規作成」を押します。
2. 「画像」でファイルまたはカメラを選び、透過状態と背景警告を確認します。
3. 「切り抜き」で背景除去、自動トリム、消しゴム・復元、位置、倍率、反転、余白を調整します。
4. 「部位候補」で端末内検出を実行します。輪郭を上部・中央・左・右・底部の候補へ分け、砲撃・被弾の動作中心と、待機・移動の接地点を選びます。
5. 「動作」で待機、移動、砲撃、被弾から選び、それぞれ3種類のプリセットを選びます。
6. 標準は高品質512pxです。必要な場合だけ「微調整」を開き、誤タップしにくい大きな `−` / `＋` ボタンで調整します。
7. 「確認」で再生・停止、背景、基準点、当たり判定候補、容量を確認します。
8. 「出力」でスプライトPNG、メタデータJSON、3ファイルをまとめたZIPを保存します。

切り抜き・部位・モーションの画像領域は縦スクロールを妨げません。モーション生成では高解像度の編集画像から一度だけ512pxへ正規化し、プレビューCanvasも端末倍率へ合わせるため、再生中だけ急に粗くなる挙動を防ぎます。Web Workerが使えない端末だけ、継続利用のため256pxの軽量代替へ自動で切り替え、画面に明示します。生成結果は元画像の決定的な2D変形のみで、新しい顔や部品は生成しません。
...
## 旧キャラクター登録・GitHub連携基盤

次のモックとGitHub App連携コードは段階移行用に保持されていますが、モーション特化版 `0.2.0` の通常画面からは呼び出しません。モーションZIPを確認した後の登録工程へ再接続するときに使用します。旧モックは次の4シナリオを扱います。
...
E2E は Chromium の Android 13 相当 User-Agent、`412 x 915`、タッチ、端末倍率 `2.625`、縦画面で、画像取り込み、画像上からのスクロール、切り抜き、部位候補検出、砲撃プリセット、高品質512px生成、ZIP保存、再起動後の下書き、オフライン復帰を確認します。初回は `npx playwright install chromium` でブラウザを用意してください。
...
- 部位候補は透明輪郭を空間分割する決定的な端末内検出です。現時点では動作中心・接地点として使い、耳、翼、武器、尻尾等を意味的に識別して個別変形する機能は未実装です。
