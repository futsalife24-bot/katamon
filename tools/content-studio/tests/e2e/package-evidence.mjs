...
await writeFile(`${output}/summary/index.html`,`<!doctype html><meta charset="utf-8"><title>Content Studio audit</title><style>body{font:16px system-ui;margin:2rem;max-width:1000px}img{max-width:100%;max-height:720px}pre{white-space:pre-wrap}</style><h1>Content Studio audit evidence</h1><p>Real frontend/backend; GitHub boundary fixtures. Chromium emulation, not real Android. No production publication. All outcomes, including failures/skips, are retained. Complete report/trace paths and SHA-256 are in manifest.json. Download the indicated details part to inspect a trace; combine indexed paths to open the original full report.</p><p><a href="results.json">Existing frontend/backend results</a> · <a href="production-results.json">Production HTTPS results</a> · <a href="manifest.json">Evidence manifest</a></p><pre>${escape(JSON.stringify(results.stats??results,null,2).slice(0,6000))}</pre>${pictures.map(p=>`<figure><figcaption>${escape(p.label)}</figcaption><a href="${p.name}"><img loading="lazy" src="${p.name}"></a></figure>`).join('')}`);
