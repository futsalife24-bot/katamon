...
for(const dir of ['tools/content-studio/playwright-report','tools/content-studio/test-results','playwright-report','test-results/playwright','test-results/registration'])await scan(dir);
try{await stat('test-results/registration-results.json');files.push('test-results/registration-results.json');}catch(e){if(e.code!=='ENOENT')throw e;}
...
try{productionResults=JSON.parse(await readFile('tools/content-studio/test-results/production-results.json','utf8'));}
catch(e){if(files.some(file=>file.replaceAll('\\','/')==='tools/content-studio/playwright-report/production/index.html'))throw new Error('Production HTML exists but production result JSON is missing or invalid.',{cause:e});productionResults={unavailable:'Production HTTPS tests did not produce results. Inspect workflow failures; this is not PASS.'};}
await writeFile(`${output}/summary/production-results.json`,JSON.stringify(productionResults,null,2));
let registrationResults;
try{registrationResults=JSON.parse(await readFile('test-results/registration-results.json','utf8'));}
catch(e){if(files.some(file=>file.replaceAll('\\','/')==='playwright-report/registration/index.html'))throw new Error('Registration HTML exists but result JSON is missing or invalid.',{cause:e});registrationResults={unavailable:'Registry Emulator/browser tests did not produce results. This is not PASS.'};}
await writeFile(`${output}/summary/registration-results.json`,JSON.stringify(registrationResults,null,2));
...
await writeFile(`${output}/summary/index.html`,`<!doctype html><meta charset="utf-8"><title>Content Studio audit</title><style>body{font:16px system-ui;margin:2rem;max-width:1000px}img{max-width:100%;max-height:720px}pre{white-space:pre-wrap}</style><h1>Content Studio audit evidence</h1><p>Real frontend/backend; GitHub boundary fixtures. Chromium emulation, not real Android. No production publication. All outcomes, including failures/skips, are retained. Complete report/trace paths and SHA-256 are in manifest.json. Download the indicated details part to inspect a trace; combine indexed paths to open the original full report.</p><p><a href="results.json">Existing frontend/backend results</a> · <a href="production-results.json">Production HTTPS results</a> · <a href="registration-results.json">Registry Emulator/browser results</a> · <a href="manifest.json">Evidence manifest</a></p><pre>${escape(JSON.stringify(results.stats??results,null,2).slice(0,6000))}</pre>${pictures.map(p=>`<figure><figcaption>${escape(p.label)}</figcaption><a href="${p.name}"><img loading="lazy" src="${p.name}"></a></figure>`).join('')}`);
