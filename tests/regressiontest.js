...
check('1vs1の名前カードは2枚・専用枠高さ56・上端44',
  panels1v1.length === 2 && panels1v1.every(p => p.h === 56 && p.cardY === 44),
  JSON.stringify(panels1v1));
check('1vs1の司令盤はターン帯・ミニマップ・HUD下端を拡張位置へ揃える',
  kt.turnBarTop() === 126 && kt.minimapTop() === 156 && kt.hudBottom() === 150,
...
check('2vs2は司令盤のターン帯とミニマップをカード2段ぶん下げる',
  kt.turnBarTop() === 164 && kt.minimapTop() === 194 && kt.hudBottom() === 188,
  `帯=${kt.turnBarTop()} 地図=${kt.minimapTop()} HUD=${kt.hudBottom()}`);
