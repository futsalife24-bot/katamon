...
(async () => {
  ...
  check('battle HUD crops transparent asset margins and uses measured content anchors',
    ...
    && anchoredHudSource.includes("const BUILD_ID = 'v2.0.55-transient-state-reset';"));
  ...
  check('2vs2も1vs1と同じ丸形コンソールへ現在・方向・予報を集約する',
    ...
    && anchoredHudSource.includes('drawUnifiedRoundWindConsole(cx, roundCardY, expanded, windLevel, windTitle, forecast, fixedForecast, forecastText);'));
  ...
  check('battle HUD reserves the centered round wind console and keeps the stage title layer clear',
    ...
    && hudSource.includes("const BUILD_ID = 'v2.0.55-transient-state-reset';"));
  const app = kt();
  const h = app.stage3();
  ...
  check('the battle wind console keeps its strength label and shows a 0-to-10 scale',
    ...
    && htmlText.includes('const windTitle = calmWind ? \'無風\' : `現在の風 ${windStrengthScale}/10`;')
