...
const evidence=path.join(root,'docs/tasks/2026-09-13-boss-motion-evidence',process.env.COOP_MOTION_BASE_URL?'published':'');
const hook=`globalThis.__motionQA={
 ...
 focus:()=>{cameraZoom=.62;cameraX=coopBossUnit.x-visibleWorldWidth()/2;cameraY=coopBossRect(coopBossUnit).y-(365-cameraStageScreenTop())/cameraZoom;cameraTargetX=null;cameraTargetY=null;},
...
(async()=>{
 ...
 try{for(const boss of ['siege-fortress-01','storm-dragon-02']){
  ...
  for(const [label,t] of [['down',.44],['up',1.31]]){
  ...
  assert.deepEqual(errors,[]);results.push({boss,realSalvo:true,neutralPoseRestored:true,errors});await context.close();
