...
const EXPECTED_BUILD_ID = 'v2.0.176-content-studio-registration';
...
function assertCacheVersionContract(html, worker) {
  ...
  assert.match(tierBody(worker, 2), /hamulton-cream-cloud-frames\.png/, '曜日迷宮で再利用する雲はオフライン準備のT2へ含めてください。');
  assert.doesNotMatch(tierBody(worker, 2), /weekday_dungeon_vault_01\.jpg/, '廃止した専用縦長背景をT2で再取得してはいけません。');
