...
- 元画像の向きは利用者が左・右から1回選ぶ。接地点と砲口は透明輪郭から端末内で推測し、画像タップで手動修正できる。被弾だけ別画像を任意選択でき、未選択時は通常画像へフォールバックする。被弾は浮上しながら右向きなら反時計回り112°、左向きなら鏡方向へ回転し、素早く元へ戻す。目や表情は検出・合成しない。
...
- 透明輪郭から接地点と砲口を端末内推測する。被弾だけ任意の専用画像へ差し替え、画像自体には描き足さない。右向きは反時計回り112°、左向きは時計回り112°を最大角とし、浮上後すぐに元の姿勢へ戻す。専用画像が復旧できない場合は生成を安全に中断する。
