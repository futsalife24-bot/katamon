...
利用者確認を受け、日常操作の主目的をキャラクター登録全体からモーション素材生成へ絞った。現行UIは「画像、切り抜き、部位候補、動作、確認、出力」の6ステップとし、基本情報、技、検証、GitHub反映は画面から外す。正規キャラクターデータとGitHub App連携の基盤コードは削除せず、モーション確認後の別工程として再接続できる状態を維持する。

- 標準出力は512px。高解像度の編集元から最終サイズへ一度だけ正規化し、矩形素材を正方形へ強制変形しない。
- Canvasの表示バッファは端末のdevice pixel ratioへ追従する。Worker未対応時だけ256px軽量代替へ切り替え、利用者へ明示する。
- 画像Canvasと調整領域は `touch-action: pan-y` とし、縦スクロールを優先する。常時表示するrange sliderは廃止し、詳細を閉じた状態の大きな増減ボタンへ置き換える。
- 待機、移動、砲撃、被弾の4動作へ、それぞれ3つの決定的プリセットを用意する。開始・終了境界は静止状態へ戻し、ループまたは単発再生の継ぎ目を安定させる。
- 部位候補検出は外部APIやAIを使わず、透明輪郭を上部、中央、左、右、底部へ分割する。MVPでは砲撃・被弾の変形中心と待機・移動の接地点に使用し、意味的な部位認識と個別変形は将来拡張とする。
- 出力ZIPはスプライトPNG、メタデータJSON、動作・品質・部位候補を含むモーションプロファイルJSONを格納する。
...
- 待機、移動、砲撃、被弾の4動作と各3プリセットを用意する。旧待機プリセット値はschema migrationで安全に読み込む。
- 透明輪郭から5つの部位候補を端末内検出する。候補は変形中心と接地点に使うが、MVPの描画は全体変形に限定する。
...
- 4動作×3プリセットの決定的モーション、512pxスプライト、メタデータ、モーションプロファイル生成。
- 端末内の部位候補検出、動作中心・接地点の選択、ゲーム相当プレビュー。
...
- E2E: 現行ChromeをAndroid 13相当の縦412×915、touch、mobile user agent、DPRで実行し、6ステップ、画像領域からの縦スクロール、部位検出、砲撃プリセット、512px生成、ZIP、再読み込み復旧、オフライン復帰まで操作する。
