...
## Completion Phase 1: 現行一覧と安全な公開

現行の正本・安全境界・監査証拠は [Phase 1記録](tasks/content-studio-completion-p1.md) を参照する。以降のMVP調査経緯は当時の観測であり、現行仕様と区別する。公開の正本はGitHubの固定SHAにあるcanonical recordsとし、表示用Pagesカタログを信用しない。

## 0.5.0 5モーションの強度調整と既存キャラクターへの後付け

日常画面をスライダーだらけに戻さず、前進、後退、単発砲撃、被弾、着地の各カードへ48dp以上の「控えめ・標準・激しめ」だけを配置する。選択値はモーションごとに下書きへ保存し、生成時に移動、上下動、回転、潰れ・伸びを安全範囲内で一括調整する。被弾は3段階すべてで90度を超えて反転し、標準の後方移動は512px出力で128px、控えめ96px、激しめ160pxとする。

現行の既存キャラクターは一括で手書きデータを移行せず、ダッシュボードの「既存キャラへモーション追加」から1体ずつ選ぶ。既存の表示名、能力、技、静止画像、画像向きは読み取り専用で保持し、Content Studioは通常画像を既存アセットから安全な許可リスト経由で読み込み、5種類のスプライトとメタデータだけを新しいハッシュ付きディレクトリへ生成する。正規レコードの `legacyTargetId` が既存IDを指し、互換カタログはゲーム側の手書きキャラクターへ `motionSheets` と `motionMetadata` だけを添付する。IDとslugが更新対象と一致しないレコードはブラウザとサーバー双方の同一スキーマで拒否する。

更新は原則1体1PRとし、未更新キャラクターは従来の静止画像表示へフォールバックする。これにより途中で追加済みと未追加が混在しても既存ゲームを壊さず、ダッシュボードの進捗表示で移行済み体数を確認できる。MVPのゲーム描画は従来どおり静止画像であり、生成した5モーションを実バトルの状態へ割り当てて再生する処理は、全キャラクター生成とは分離した次段階のゲーム本体変更とする。

更新日: 2026-08-18

## 0.4.4 モーション特化フロー

利用者確認を反映し、日常UIを「画像、向きと基準点、生成、キャラ、GitHub」の5ステップへ縮小した。モーション調整用のプリセット選択とスライダー群、能力・必殺技の入力、安定しない目の推測・合成は日常画面から外す。一方、キャラクター追加に必要な最小情報とGitHub自動反映は残し、1体を最後まで登録できる縦のフローを維持する。

- 通常画像から前進、後退、単発砲撃、被弾、着地の5クリップを決定的な固定設定で一括生成する。前進・後退は向きを変えずその場で動き、ゲーム側がワールド座標を移動させる。
- 元画像の向きは利用者が左・右から1回選ぶ。接地点と砲口は透明輪郭から端末内で推測し、画像タップで手動修正できる。被弾だけ別画像を任意選択でき、未選択時は通常画像へフォールバックする。被弾は浮上しながら右向きなら反時計回り112°、左向きなら鏡方向へ回転し、標準512px出力では約半身分に当たる128px後方へ吹き飛ぶ。軽量出力では出力辺長に比例させる。反転姿勢のまま着地して低く1回バウンドし、接地状態を数フレーム保ってから元位置へ戻る。画像変形と独立して当たり判定候補は通常姿勢の位置へ固定する。目や表情は検出・合成しない。
...
調査開始時の基準は `origin/master` の `v137` である。既存の未コミット作業を保護するため、実装は正本から分離した `feat/content-studio-mvp` worktreeで行った。2026-08-18にGitHub正本の `origin/master`（v2.0.76）を改めて統合し、17体構成と新しい `assets/characters/{runtime,master}/` 配置へ追従した。
...
現行の既存キャラクターは一括移行しない。現在の情報は主に次へ分散している。
...
画像は `assets/characters/runtime/{asset}.webp` を優先し、`assets/characters/master/{asset}.png` へフォールバックする。既存ゲームは静止画像を高さ78px相当、アスペクト比維持、底面中央で描画する。待機スプライト、GIF、アニメーションWebPは現時点では使っていない。
...
現行の既存キャラクターは手書き定義として残す。新規キャラクターだけを `content/characters/{slug}.json` へ保存し、生成カタログを既存データへ明示的にマージする。
...
- Canvas再エンコードでEXIF等を除去する。通常の元画像と任意の被弾用元画像は、下書き内で別Blobとして保持できる。
...
- 透明輪郭から接地点と砲口を端末内推測する。被弾だけ任意の専用画像へ差し替え、画像自体には描き足さない。右向きは反時計回り112°、左向きは時計回り112°を最大角とする。12フレームで後方への浮上反転、反転姿勢を維持した着地、低い1回のバウンド、後方接地位置での待機、元位置への復帰に分け、輪郭下端で着地位置を補正する。当たり判定候補は0フレーム目の通常姿勢から算出して固定する。専用画像が復旧できない場合は生成を安全に中断する。
- 下書きschema v5は被弾用画像の情報と `hit-original` Blobを追加する。v4以前の下書きは通常画像を使い、旧い目位置データを捨てて安全に移行する。
...
MVPで既存ゲームへ加える変更は、生成カタログの読み込み・安全なマージ、宣言型技ディスパッチ、StudioパスのService Worker除外、対応テストだけに限定する。現行の既存キャラクターのデータ、画像、処理順を変更しない。
...
- 端末内の接地点・砲口の推測、画像タップ補正、任意の被弾用画像、向きに応じた112°被弾回転、ゲーム相当プレビュー。
...
- E2E: 現行ChromeをAndroid 13相当の縦412×915、touch、mobile user agent、DPRで実行し、5ステップ、通常画像と被弾用画像の保存復旧、画像領域からの縦スクロール、接地点・砲口補正、被弾112°回転を含む5クリップの512px生成、生成ボタン固定、ZIP、モックPRと安全マージ、再読み込み復旧、オフライン復帰まで操作する。
