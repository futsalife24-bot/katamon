...
既存16体は一括で手書きデータを移行せず、ダッシュボードの「既存キャラへモーション追加」から1体ずつ選ぶ。既存の表示名、能力、技、静止画像、画像向きは読み取り専用で保持し、Content Studioは通常画像を既存アセットから安全な許可リスト経由で読み込み、5種類のスプライトとメタデータだけを新しいハッシュ付きディレクトリへ生成する。正規レコードの `legacyTargetId` が既存IDを指し、互換カタログはゲーム側の手書きキャラクターへ `motionSheets` と `motionMetadata` だけを添付する。IDとslugが更新対象と一致しないレコードはブラウザとサーバー双方の同一スキーマで拒否する。

更新は原則1体1PRとし、未更新キャラクターは従来の静止画像表示へフォールバックする。これにより途中で16体が混在しても既存ゲームを壊さず、ダッシュボードの進捗表示で移行済み体数を確認できる。MVPのゲーム描画は従来どおり静止画像であり、生成した5モーションを実バトルの状態へ割り当てて再生する処理は、全キャラクター生成とは分離した次段階のゲーム本体変更とする。
