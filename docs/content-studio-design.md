...
- 元画像の向きは利用者が左・右から1回選ぶ。接地点と砲口は透明輪郭から端末内で推測し、画像タップで手動修正できる。被弾だけ別画像を任意選択でき、未選択時は通常画像へフォールバックする。被弾は浮上しながら右向きなら反時計回り112°、左向きなら鏡方向へ回転して後方へ吹き飛ぶ。反転姿勢のまま着地して低く1回バウンドし、接地状態を数フレーム保ってから元位置へ戻る。画像変形と独立して当たり判定候補は通常姿勢の位置へ固定する。目や表情は検出・合成しない。
