...
固定3部位（砲身の固定攻撃、装甲の固定HP、コアの固定防御）の本番値は未確定である。Phase 1では注入されない限り生成・GS計算を明示エラーで停止するfail closedとし、値を推測して埋めない。
...
| 必須 | 固定3部位値 | 砲身固定攻撃、装甲固定HP、コア固定防御の本番値が未確定 | Phase 1はfail closed。Phase 2C/2Dの報酬生成前に、必要な互換性依存を解決して値を承認する。 |
...
**完成条件**: 同じ入力は常に同じ完全ギアを返し、丸め・上限が明示され、固定3部位の未確定値はfail closedで扱う。戦闘実測は含めない。
...
固定3部位の本番tuningは未確定である。Phase 2A/2Bは保存・受取基盤だけを先行できるが、完成ギアを生成するPhase 2C/2Dの前に、砲身・装甲・コアの固定メイン値と必要な互換性依存を解決する。
...
- Phase 1の数値表・Seed・境界設計: **Sol High**。正本の数値を変えず、未確定値をfail closedで隔離する重要設計。
...
砲身・装甲・コアの固定3部位の★別完成値は、Phase 1では未注入・fail closedのままとする。Phase 2A/2Bはこの未確定事項と独立して進められるが、Phase 2C/2Dの完成ギア報酬の前に、このtuningと必要な互換性依存を承認する。Phase 3の戦闘反映は、Phase 3の判断事項をすべて承認後に進める。
