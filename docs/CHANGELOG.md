...
## v2.0.174 曜日ダンジョン既存BATTLE統合（公開済み・PR #384、2026-09-05）
...
- 公開確認: PR #384をmerge commit `0e033a128fd291ae2ecea085c45cdfb1ad5f8057`でmasterへ統合し、GitHub Pages run `33910978605`のbuild / deploy成功後、公開`index.html`と`sw.js`の双方が`v2.0.174-weekday-battle-dungeon`を返すことを確認した。

## v2.0.173 曜日ダンジョン（2026-09-04）

- GARAGEへJST 0時更新・1日1射の専用チャレンジを追加した。月〜土は砲身・装甲・コア・動力・照準・補機を順に固定し、日曜だけ6部位から選べる。
- 命中時は対象部位Gear 1個、外れ時は粉末3個を既存の未受取報酬へ保存する。入場・照準・退出では消費せず、FIRE保存と読戻しが成立した時だけ当日分を確定する。
- 専用Web Lock、JST日付再検証、最大消費日の単調保存、Gearキュー／台帳の耐久確認により、同日複数タブ、時計巻戻し、発射・報酬保存中の再読込をfail closedで復旧する。
- 生成した歯車庫背景を720×1280 JPEGへ最適化してPWA Tier 2へ登録した。通常戦闘、CPU連勝、希少個体、資源箱、ONLINE/Firebaseは変更していない。`BUILD_ID`と`CACHE_VERSION`は`v2.0.173-weekday-dungeon`で一致する。

## v2.0.143 新タイトル背景・ロゴ演出（2026-08-25）

- Google Driveの提供素材を、ロード完了後の石壁タップを起点として「始点静止画→約3.04秒のロゴ動画→終点静止画」の順で一度だけ表示する。石壁が崩れている間も奥で動画が進み、終了後は終点画像をタイトル背景として固定する。
- 背景とロゴが合成済みの新素材へ一本化し、端末の実時間による朝・昼・夕・夜のタイトル背景切替と旧タイトルロゴの重ね描きを撤去した。演習設定は終点背景を暗く敷いて既存UIの可読性を保つ。
- 1080×1918の始点・終点PNGは、スマホ表示で見た目を確認して高品質JPEGへ変換し、合計約6.2MBから約0.91MBへ削減した。動画約4.17MBを含む新規3素材だけを永続キャッシュへ追加し、既存のキャラ・BGM・HUD素材は再取得しない。
- BATTLE／GARAGEの2ページ、木板UI、各画面遷移、BGM、戦闘、オンライン、協力ボスは変更しない。`BUILD_ID`と`CACHE_VERSION`は`v2.0.143-title-logo-intro`で一致する。

## v2.0.142 タイトルBATTLE／GARAGE 2ページ化（2026-08-24）

- タイトルの背景・ロゴ・サウンド・更新履歴・build表示を固定し、中央の既存木板だけを320msのease-outで横スライドする2ページ構成へ変更した。黒鉄のBATTLE／GARAGEタブ、左右矢印、左右スワイプに対応する。
- BATTLEはCPU BATTLE、ONLINE BATTLE、チュートリアル、演習、RANKING。GARAGEはショップ、実績、サウンドテストを収録する。CPU中断表示と既存の各画面遷移は維持した。
- 既存の羊皮紙・矢吊り看板・緑盾・木板を再利用し、新規画像は追加していない。サウンドテスト画面からショップと実績を撤去し、BGM一覧と再生操作だけへ戻した。
- 中央メニューはデータ配列化し、将来のギア装備・倉庫・プリセット・ガイドはGARAGEのitemsへ追加できる。短いスワイプ、縦ずれ、2本指、pointercancel、端末戻るによる誤発火を回帰テストで防止する。
- `BUILD_ID`と`CACHE_VERSION`は`v2.0.142-title-menu-pages`で一致する。画像素材は変更していないため、既存端末は永続キャッシュの大型素材を再取得しない。

## v2.0.141 演習プレビューの可視サイズ・接地統一（2026-08-24）

- 演習設定の味方・敵キャラは、戦闘と同じ面積補正で表示し、カード内の可視足元を下端へ揃えた。縦長の素材が小さく見えたり、透明余白のために上へ浮いて見えたりしない。
- クール=カイ、ドレッドアロー、ハムルトンは、原画の可視車体を含む範囲だけを演習カードへ切り出す。戦闘の物理座標・当たり判定・表示倍率は変えない。
- `BUILD_ID`と`CACHE_VERSION`は`v2.0.141-practice-preview-grounding`で一致する。画像ファイルは変更していないため、既存端末は永続キャッシュのキャラ素材を再取得しない。
- 旧v2.0.140では新しい演習プレビュー契約が不成立であることを確認後、Regression p1/e1各442件、協力ボス実戦129/129、全体`npm.cmd test`を成功させた。

## v2.0.140 静的キャラ表示の右向き原画統一（2026-08-24）

- 演習のキャラ表示を含む全画面の反転処理を監査し、敵欄だけを左へ反転していた演習設定の処理を撤去した。味方・敵ともに、全員右向きへ統一済みの原画をそのまま表示する。
- キャラクター選択、ローダー、ONLINEロビー、必殺カットインはすでに原画を直接表示していることを確認した。対戦フィールド、跳躍、協力UI、対戦開始VSに残る反転は、対戦相手を向かせる陣営共通の表現であり、素材ごとの例外補正ではない。
- `BUILD_ID`と`CACHE_VERSION`は`v2.0.140-static-preview-orientation`で一致する。画像素材は変更していないため、既存端末は永続キャッシュのキャラ画像を再取得しない。
- 旧v2.0.139の静的契約が不成立、新v2.0.140で成立することを確認してから、Regression p1/e1各440件と`npm.cmd test`全成功を確認した。実ブラウザのAndroid縦画面12件も成功。ローカルのiPhone相当WebKitは、既知のゲームCanvas起動時クラッシュによりStage Studioの2件だけ失敗し、今回の演習表示・Android本体導線は通過している。

## v2.0.139 全キャラ接地・右向き原画統一（2026-08-24）

- 全18体の表示サイズと可視足元を監査し、物理足元・爆風・命中判定は動かさず、透明余白で浮いて見えたアイボルト／ゴーロッカ／ドレッドアロー／ハムルトン／クール=カイの描画接地だけを補正した。跳躍中の車体、手番表示、協力UI表示も同じ値で追従する。
- ハムルトンの黄色クリーム雲は、半透明・半径76px・照準妨害を維持したまま、3コマの可視下端が地面へ触れる位置に直した。地中まで広がっていた床の発光は、雲の接地影へ縮小した。
- スモエル／ルビデビ／パラディエ／にゃんタンクのPNG原画とWebP配信用素材を正式に右向きへ統一し、ゲームのキャラ別反転補正を撤去した。顔アップの座標も反転後の原画に合わせている。
- Service Workerはこの8素材だけを改訂取得するため、既存端末は他のキャラ画像や大型素材を再ダウンロードしない。BUILD_IDとCACHE_VERSIONはv2.0.139-character-grounding-orientationで一致する。

## v2.0.138 ハムルトン雲の妨害表示・大きさ修正（2026-08-24）

- 黄色クリーム雲の範囲内で照準した時、POWERと仰角を隠して「視界不良」と表示するよう修正した。弾道ガイドは従来どおり半分の長さに制限される。
- 雲スプライトは実判定へ収まる表示倍率へ下げ、半透明（50%）にして中にいるキャラも読めるようにした。`BUILD_ID`と`CACHE_VERSION`は`v2.0.138-hamulton-cloud-debuff`で一致する。

## v2.0.137 ハムルトンのクリーム雲3コマアニメ（2026-08-24）

- ハムルトンの必殺「まさかのホウヒ」の黄色クリーム雲を、生成した透明PNGスプライトへ置き換えた。3コマをその場で繰り返して、当たり判定を動かさずにモクモクと膨らむ表現にした。
- 素材は必殺が初めて表示される時だけ読み込み、以後はService Workerの端末キャッシュを再利用する。通常プレイで新素材を先読みしないため、既存プレイヤーへ不要な初期通信を増やさない。`BUILD_ID`と`CACHE_VERSION`は`v2.0.137-hamulton-cream-cloud`で一致する。

## v2.0.136 協力ボスNORMAL戦テンポ短縮（2026-08-24）

- NORMALの基礎ボスHPを2200から1650、ラウンド制限を20から12へ変更した。部位耐久は本体HP比率のまま連動するため、部位破壊→CORE露出→Phase 2→ミサイル部位→COREの攻略構造は維持する。
- 装甲30、部位45＋本体18、NORMAL CORE90、CORE露出2要塞ラウンド、Phase 2のHP50%境界は変更しない。4人が部位・COREを優先し、5巡目にキャラ固有必殺を使う標準シミュレーションで9巡決着を固定した。HARD／EXTREMEのHP・制限・CORE設定は変更していない。`BUILD_ID`と`CACHE_VERSION`は`v2.0.136-coop-normal-tempo`で一致する。

## v2.0.135 超大型要塞の迫力・高所攻略強化（2026-08-24）

- ライブ4vs1の要塞を430×286から560×372へ大型化し、画像・本体判定・部位判定・CORE狙点を同じ比率で拡張した。
- 車体座標を固定したまま、後部排煙・足元の粉塵・機関室の脈動・主砲の圧力リング・油圧震動線が動く待機演出を追加した。見た目と命中位置がずれる全身移動は行わない。
- 既存の鋼鉄3台座と破壊可能2足場を維持し、さらに高所へ破壊可能足場2枚を追加した。`BUILD_ID`と`CACHE_VERSION`は`v2.0.135-colossal-fortress-idle`で一致する。

## v2.0.134 既存素材キャッシュの無通信移行（2026-08-24）

- v2.0.131以前の版別キャッシュに保存済みの大型素材を、ネットワークから取り直さず端末内で永続キャッシュへコピーする移行処理を追加した。
- 永続キャッシュにも旧版キャッシュにも無い素材だけを初回取得する。`BUILD_ID`と`CACHE_VERSION`は`v2.0.134-legacy-cache-migration`で一致する。

## v2.0.133 大型素材の差分更新（2026-08-24）

- 更新のたびに約44.4MiBの既存画像・音声・動画を再取得していた構造を改め、大型素材38件を端末の永続キャッシュから再利用するようにした。
- アプリコードは版ごとに更新し、素材は初回または明示した差し替え対象だけを取得する。変更素材はファイル単位で再取得し、未変更素材は通信しない。`BUILD_ID`と`CACHE_VERSION`は`v2.0.133-persistent-asset-cache`で一致する。

## v2.0.132 協力ボス準備画面のBGM修正（2026-08-24）

- 協力ボス入口・作戦準備室をBGMディレクターのルーム画面判定へ追加し、タイトル曲へ戻らず通常ONLINEと同じルームBGMを維持するよう修正した。
- 協力画面を閉じた時はタイトルBGM、戦闘開始時は要塞戦BGMへ切り替える既存遷移を維持する。`BUILD_ID`と`CACHE_VERSION`は`v2.0.132-coop-room-bgm`で一致する。

## v2.0.131 協力ボスMVP正式公開（2026-08-24）

- Firebase Rules反映と実環境書き込み確認、PR #277の全CI成功を受け、協力ボスを公開ホストで標準ONにした。
- URLパラメータなしで協力ボス入口を利用できる。緊急時は`KATAMON_FEATURES.coopBossMvp = false`で停止できる。
- `BUILD_ID`と`CACHE_VERSION`は`v2.0.131-coop-production-enable`で一致する。

## v2.0.130 協力ボス・味方AIモンスター選択（2026-08-24）

- 協力ボスの作戦準備室へ、ホスト専用のP2・P3・P4味方AIモンスター選択欄を追加した。AI補充ONの空席は、ホストが席ごとに選んだモンスターで出撃する。
- 人間が参加した席は参加者本人のモンスターを優先し、空席へ戻った時は保存済みのAI選択へ戻す。AI補充OFF・通常ONLINE・通常CPU戦の編成は変更しない。
- AI選択は協力部屋の`settings.aiCharacters`へ保存し、Firebase Rulesで既存18キャラだけを許可する。`BUILD_ID`と`CACHE_VERSION`は`v2.0.130-coop-ai-character-select`で一致する。

## v2.0.129 一斉必殺の同時オーラ・跳躍・救助弾（2026-08-24）

- ソロ一斉行動で複数必殺を選んだ時は、参加キャラ全員へ0.9秒のオーラを同時表示し、一呼吸置いてから2.37秒の順次カットインを開始する。実弾はカットイン終了後だけ約150ms間隔で発射する。
- READY入力へ跳躍と救助弾を追加した。跳躍は着地後移動不能と使用権、救助弾は1試合1回の残弾を、通常手番と同じ発射処理で実際の発射時に1回だけ消費する。SUBと救助以外の協力アイテムは一斉行動へ混ぜない。
- 通常戦、複数人協力戦、Firebase Rulesは変更しない。`BUILD_ID`と`CACHE_VERSION`は`v2.0.129-coop-salvo-aura-support`で一致する。

## v2.0.128 同時必殺・順次カットイン演出（2026-08-24）

- 同時必殺カットインを1.58秒から正確に1.5倍の2.37秒へ延長した。各キャラパネルは約0.16秒差で左右から順番に飛び込み、全員が揃ってから見出しを表示する。
- `同時必殺`を34pxから48pxへ拡大し、黒鉄グラデーションと真鍮枠の専用銘板を追加した。キャラ絵・技名・キャラ名・固有色・共通SEと実弾処理は維持する。`BUILD_ID`と`CACHE_VERSION`は`v2.0.128-coop-special-salvo-stagger`で一致する。

## v2.0.127 協力一斉必殺・タイトル退出修正（2026-08-24）

- ソロ＋CPU3体の一斉攻撃で通常弾に加えてゲージMAXの必殺を選べるようにした。複数人が必殺を選んだ場合は、既存キャラ絵を人数ぶん同時に見せる黒鉄・真鍮・キャラ色の専用カットインを挟み、各必殺を約150ms間隔で実発射する。
- 必殺カットインSEをメロニキ提供の`special-cutin-finisher.mp3`へ差し替えた。同時必殺では共通SEを人数ぶん重ねず1回だけ鳴らし、クール=カイの既存ボイスだけを重ねる。
- 協力戦のメニュー「タイトルへ戻る」が作戦準備室を再表示していた経路を分離した。ホストは部屋全体、参加者は自席を片付けてタイトルへ戻り、リザルトの「ロビーへ戻る」は従来経路を維持する。`BUILD_ID`と`CACHE_VERSION`は`v2.0.127-coop-special-salvo-title-exit`で一致する。

## v2.0.126 協力戦・鋼鉄台座と破壊可能足場（2026-08-23）

- 2160×960の地面と初期配置に使う空中3台座は鋼鉄のまま維持し、右側の移動用2足場を通常砲撃で破壊可能へ変更した。鋼鉄と破壊可能地形は色でも判別できる。
- 要塞直前の足場を左へ短縮して車体前の射線と移動空間を拡大した。NORMALの命中精度や攻撃倍率は変えず、既存のボス弾でも遮蔽物を崩せる地形を先に評価する。地形貫通弾は未導入。
- mixed-material開始snapshotの通信検証を追加し、旧・全面鋼鉄の空中足場構成を拒否する。台座の非破壊・移動用足場の破壊・通常戦回帰を自動検査した。`BUILD_ID`と`CACHE_VERSION`は`v2.0.126-coop-mixed-terrain-clearance`で一致する。

## v2.0.125 一斉攻撃・確定ガイド保持（2026-08-23）

- ソロ一斉攻撃でプレイヤーがREADY確定した白い弾道ガイドを、CPU3体の入力待ちから4門READYのカットイン終了まで戦場へ残す。
- CPU味方の弾道ガイドは表示せず、答えを見せないことと将来の複数人戦で狙いを秘匿するルールを維持する。一発目の発射開始と同時に確定ガイドを消す。
- 弾道・ダメージ・READY順・150ms間隔の発射処理は変更しない。Android Chromium E2EでREADY後の表示保持と一斉発射後の非表示を確認した。`BUILD_ID`と`CACHE_VERSION`は`v2.0.125-coop-salvo-own-guide-hold`で一致する。

## v2.0.124 協力NORMAL延長・ソロ一斉攻撃（2026-08-23）

- 協力NORMALを18ラウンドから20ラウンド（最大100手）へ延長する。HARD 15ラウンド／EXTREME 12ラウンドとボスHP・装甲・CORE倍率は維持し、部位を狙う価値を落とさず討伐猶予だけを増やした。
- ソロ＋CPU3体では4体が通常弾の照準をREADYし、通常2vs2と同じ投射物を固定物理0/18/36/54tick（約150ms間隔）で発射する。READYでは手番効果を先行消費せず、全弾解決後にだけ要塞手番へ進む。
- 第一段階は通常弾限定とし、必殺・跳躍・SUB・CO-OP ITEMを混在させない。先頭弾で撃破しても予約弾を最後まで処理する。複数人戦は専用packet・権威検証を導入するまで逐次手番を維持する。
- 2160×960・720列の立体steel開始snapshotと難易度別手数を通信境界でも受理するよう揃え、Android Chromium E2EでREADY 0/4から4/4、全弾解決、要塞手番まで確認した。`BUILD_ID`と`CACHE_VERSION`は`v2.0.124-coop-solo-salvo-turn-balance`で一致する。

## v2.0.123 協力大型立体ステージ・跳躍毎手番化（2026-08-23）

- 協力4vs1を既存の大型ステージ基盤（2160×960）へ移し、壊れない鋼鉄地面と高さの異なる空中足場5つを同じ`solidGrid`へ載せた。味方4体は地面・足場へ分かれて出撃し、通常の縦横カメラ追従と跳躍で段差を移動できる。
- 跳躍はCPU戦・ONLINE・演習・チュートリアル・協力戦の全モードで、自分の手番開始時に使用可能へ戻す。着地後の移動不能は従来どおり次の自手番を封印するため通常は実質2手番に1回だが、2手番チャージは持たせず将来のデバフ解除へ対応できる構造にした。
- 協力戦の跳躍と救助ボタンをそれぞれ13px／14px上へ移し、中央砲座や互いのタップ領域とは重ねない。跳躍ボタンの残数表記は旧`1回`から`使用可`へ変更した。
- 大型寸法、5足場、全面steel、非破壊、手番開始時の使用権復帰と移動不能優先を自動検査へ追加。`BUILD_ID`と`CACHE_VERSION`は`v2.0.123-coop-vertical-jump-refresh`で一致する。

## v2.0.122 要塞次弾予告・救助直後保護・協力操作配置（2026-08-23）

- 要塞の標的を距離依存から席順巡回へ変更し、戦場の照準枠と手番バーへ次弾の攻撃種・対象を表示する。救助直後の味方は他に生存者がいる間、直後の要塞砲撃対象から外す。
- 救助された本人に同じ味方巡回の未消化手番が残っていても待機させ、次の味方巡回から通常行動へ戻す。全員がダウンし保護対象しか残らない場合は要塞が攻撃できるため、無敵化はしない。
- 弾道ガイドをキャラ・要塞より前へ描画し、協力戦の跳躍を左下、救助をその右下、必殺を右側の枠内へ移した。通常1vs1／2vs2の操作座標は変更しない。
- 空の実績通知は引き続きDOMごと非表示にし、Android相当582×1177で上端の金枠が再発しないことを確認した。`BUILD_ID`と`CACHE_VERSION`は`v2.0.122-coop-telegraph-revive-guard`で一致する。

## v2.0.121 画面上端の空通知枠修正（2026-08-23）

- 協力MVPの実績通知が空のまま常駐し、画面外へ120%ずらしても下端だけ見えていたため、初期状態・空文字・退場後はDOMごと非表示にした。
- 実績達成時だけ通知を表示し、2.6秒後の退場アニメ完了後に内容を消して再び隠す。通知機能そのものは維持する。
- Android相当582×1177の実ブラウザで、空通知が`display:none`・0×0になり、タイトル上端の金枠が消えることを確認した。`BUILD_ID`と`CACHE_VERSION`は`v2.0.121-coop-toast-empty-frame`で一致する。

## v2.0.120 露出CORE命中・狙点サイト改善（2026-08-23）

- 露出COREへ正しく向かう通常弾は外殻の開口部を通ってCOREへ届くようにした。NORMALでは通常直撃45の2倍となる90ダメージを与える。
- COREを外した通常弾は従来どおり外殻へ衝突し、装甲本体30ダメージとなる。大型要塞全体を貫通可能にはしない。
- 部位サイトを実際の当たり判定サイズへ合わせ、二重リング、照準線、中央点、発光、部位名を強調した。露出中のCOREは`CORE OPEN`と点滅リングで区別する。
- 実投射物の回帰テストで「通常弾が露出COREへ90」「狙いを外すと装甲へ30」を固定した。`BUILD_ID`と`CACHE_VERSION`は`v2.0.120-coop-core-target-reticles`で一致する。Firebase Rulesの変更はない。

## v2.0.119 協力戦の跳躍着地・救助弾・操作盤配置修正（2026-08-23）

- 跳躍弾が大型要塞へ直撃した時と、低弾道で要塞直下の鋼鉄床へ先に触れた時は、着弾座標へ機体を重ねず進入側の車体外へ安全着地する。通常1vs1／2vs2の跳躍挙動は変更しない。
- 救助弾は大型要塞・生存中の味方を通過し、ダウン中の味方へ触れた時だけ解決する。要塞付近で倒れた味方も救助できる。
- 協力戦の跳躍を操作盤左上、救助をその右下へ再配置し、必殺ボタンを8px下げて黄色い外周が欠けないようにした。描画座標・入力判定・UI契約を同時に更新した。
- 自動テストで要塞外着地の距離、救助対象フィルター、ボタン同士と中央砲座の非重複を固定し、Android縦画面の実ブラウザで安全着地とConsole error 0件を確認した。
- `BUILD_ID`と`CACHE_VERSION`を`v2.0.119-coop-jump-rescue-controls`へ揃えた。Firebase Rulesの追加変更はない。

## v2.0.118 協力要塞の装甲・部位・CORE・Phase 2（2026-08-23）

- 要塞の透明余白を含む矩形当たり判定を、車体＋連装砲・主砲・前面装甲・ミサイルポッドの固定複合形状へ変更した。
- 直接着弾45を装甲本体30、部位45＋本体18、露出COREはNORMAL 90へ分岐。地面爆風は通常戦の距離減衰を維持し、部位破壊でCORE露出と要塞攻撃の弱体化が起きる。
- HP50%以下の次の要塞手番をPhase 2変形へ消費し、画像、CORE再露出、強化ミサイル、警告カットイン、BGMを同じターン境界で切り替える。NORMALのCOREは要塞2ラウンドで閉じる。
- 協力戦だけミニマップを外し、部位TARGET盤と手番`▼`を追加。視点距離は全モードでHUD下の戦場側へ移し、跳躍・CO-OP ITEM・必殺・SUB・左右移動を見た目・タップ判定とも重ならない配置にした。
- 部位・CORE状態を開始／途中snapshotで検証し、Phase 2への一方向の通信遷移だけ要塞変形1手ぶんを許可する。一斉攻撃は通常4vs1の回帰完了後の別仕様として保留する。
- `BUILD_ID`と`CACHE_VERSION`を`v2.0.118-coop-armor-core-phase2`へ揃える。Firebase Rulesの手動反映とAndroid／PC複数端末QAは公開前の必須作業として残る。

## v2.0.116 協力4vs1の同期・切断復帰監査（2026-08-23）

- 通常対戦エンジンへ載せた協力4vs1の`fire→state/result`を48桁の行動IDで結び、順不同・別手番・古い状態の適用を拒否する。
- RTDBメッセージをサーバー発番キー＋小分けcursor読込へ変更し、毎回の全履歴取得と再読込時の古い砲撃再生を防いだ。
- 人席が途切れた場合はホストAIへ権威を移し、飛翔中または解決済みの行動を同じIDのまま完了する。復帰は進行中の行動後へ保留する。
- 4人VS表示、キャラ名復元、オンライン中のRキー無効化、要塞矩形へのEMP／炎判定、全キャラ定義の共有、現行通常戦fixtureを監査・補強した。
- `BUILD_ID`と`CACHE_VERSION`を`v2.0.116-coop-normal-sync-audit`へ揃えた。Firebase Rulesの手動反映と複数端末QAは引き続き公開前の必須作業である。

## v2.0.115 協力戦を通常ターン制4vs1へ統一（2026-08-23）

- ライブ協力戦から専用の一斉入力Surfaceを外し、通常2vs2のHUD、操作盤、投射物、着弾、手番終了、カメラをそのまま使う`P1→P2→P3→P4→BOSS`の4vs1へ置換した。
- 味方4体と大型要塞1体を通常`units`へ登録し、1440×660の平坦な鋼鉄床を全面非破壊にした。全景への強制縮小をやめ、標準倍率と横スクロールへ戻した。
- ソロ＋CPU3体は戦闘パケット不要のローカル権威で出撃できる。複数人戦は`coopRooms`内の`net`外包みで通常パケットを同期し、古いFirebase Rulesなら認証切れと誤表示せず明示的に中断する。
- 実ブラウザで、ソロ出撃、4体表示、横スクロール、通常弾着弾によるボスHP`1870→1825`、次手番、ボス実弾、鋼鉄床維持、新規Console error 0件を確認した。
- `BUILD_ID`と`CACHE_VERSION`を`v2.0.115-coop-normal-engine-4v1`へ揃えた。複数人実機QA前に`database.rules.json`をFirebase Consoleへ反映する必要がある。

## v2.0.112 NORMALのCORE露出時間を仕様へ統一（2026-08-22）

- 難易度カタログに残っていたNORMALのCORE露出3ラウンドを、仕様どおり2ラウンドへ修正した。
- 実際のボスAIは既に2ラウンドだったため、戦闘進行ロジックは変更していない。
- NORMALの2ラウンド経過後にCOREが閉じることを自動テストと実機QAへ明記した。

## v2.0.111 協力戦の救助弾を1試合1回へ変更（2026-08-22）

- 救助弾の正式仕様を、1人につき1試合2回から1回へ変更した。
- 使用後の同一試合での再使用を禁止し、次の試合・再戦では1回へ戻す。
- カタログ、戦闘処理、ショップ説明、仕様差分、実機QA、自動テストを同じ回数へ統一した。

## v2.0.88 退出確認ボタンの再調整（2026-08-20）

- 青緑と羊皮紙のボタンを、横長・文字なし・透過の専用画像へ差し替えた。
- 白い外枠、盾素材の横潰れ、ボタン文字の埋もれを解消した。

## v2.0.87 退出確認UIの再調整（2026-08-20）

- 前版の黒鉄・古木・真鍮の生成フレームとボタンを取り下げ、既存の木製ボード・透過済み盾ボタン・羊皮紙ボタンへ差し替えた。
- 怖すぎる装飾、白い切り抜き枠、ボタン上の文字の埋もれを解消した。
- 退出確認の動作、文言、2択の意味は変更していない。

## v2.0.86 退出確認UI素材リメイク（2026-08-20）

- 退出確認の枠と2つのボタンを、黒鉄・古木・真鍮の生成画像素材へ置換（v2.0.87で取り下げ）。
- レバー風のCSS描画を撤去し、ゲーム側の日本語テキストを素材の余白へ重ねる構成へ変更。

## v2.0.85 退出確認画面リメイク（2026-08-19）

- 端末の戻る操作で出る退出確認を、石壁・黒鉄・真鍮と新規退出門シールで構成した画面へ変更。
- 退出確認の動作と既存文言は維持し、PWAキャッシュ版を更新。

## v2.0.84 中断データ再開のタッチ修正（2026-08-19）

- キャラ選択の再開ギアと中断確認ボタンをpointerup確定へ統一し、スマホで中断データを再開できない問題を修正。

## ランキング行の手動非表示

- `gas/ranking.gs` に `hidden` 列を追加し、管理者が `TRUE` を入力した行をゲームのランキング表示から除外できるようにした。
- 空欄・`FALSE`・`0`は通常表示として扱う。スコア送信、順位計算の元データ保存、HMACなしの現行方針は変更しない。
- 反映にはApps Script側で `ranking.gs` を全量貼り付け、新バージョンをデプロイする必要がある。

## v2.0.77 クール=カイ連戦BGM継続（2026-08-18）

- 「つぎのバトルへ」では専用BGMを止めず、同じ音源・再生位置から継続する。
- タイトルへ戻った後の新しい対戦では、専用BGMを先頭から再生する。

## v2.0.76 BGMサウンドテスト（2026-08-18）

- 起動ローダーでタイトル・ONLINE・全ステージ・おまけ1〜5を先読みし、進捗へ反映。
- おまけを一覧式サウンドテストに変更し、全BGMを選んで再生できるようにした。

## v2.0.75 ローダー配置（2026-08-18）
...
## v2.0.71 オベリスク必殺「プリズムビーム」（2026-08-18）

- 地形を壊さず敵を貫通する反射レーザーへ変更。
- 最大4回の反射または1320pxの射程で、地形を壊さず爆発する。

## v2.0.70 クール=カイ演習画像の接地後切り出し（2026-08-18）

- 演習選択のクール=カイだけ、上下透明余白を含めない専用切り出しへ更新。
- バトル中の接地位置・キャラクター性能は変更しない。

## v2.0.69 初回BATTLE準備とデバフ状態表示（2026-08-18）

- 初回タップ時にBGMとBATTLE START動画を無音再生でウォームアップし、初回BATTLEの再生・描画遅延を抑制。
- BATTLE画面に全ユニットのデバフ名と残りターンを表示。

## v2.0.68 握り飯の個別ランダム回転（2026-08-18）

- クール=カイの握り飯47発それぞれへ、見た目だけの再現可能なランダム回転を追加。
- 弾速・ランダム角度・連射間隔・ダメージ・判定半径・地形破壊・移動封印は変更しない。

## v2.0.67 クール=カイへ名称変更（2026-08-18）

- キャラクターの表示名を「クールカイ」から「クール=カイ」へ変更。
- キャラクターキー、素材、必殺技、性能、専用BGMは変更しない。

## v2.0.66 クールカイ必殺後の自己移動封印（2026-08-18）

- 必殺技「Amour 握り飯」の発動後、クールカイ自身へ6ターンの移動封印を付与。
- 47発の連射、弾の判定・ダメージ・地形破壊、専用BGMは変更しない。

## v2.0.65 クールカイ火力・接地調整（2026-08-18）

- クールカイの最大HPを66へ変更。
- 握り飯1発のダメージを6へ変更。見た目3倍・判定半径3・小さな地形破壊は維持。
- クールカイの見える絵を地面アンカーへ31px寄せ、バトル中の浮きを補正。

## v2.0.64 クールカイ表示・弾サイズ調整（2026-08-18）

- 演習のキャラ表示にも透明余白の切り出しを適用し、クールカイを他キャラと同じ見かけサイズへ調整。
- バトル中のクールカイ画像を地面アンカーへ7px寄せて、浮いて見える状態を補正。
- 握り飯は見た目だけ3倍に拡大。判定半径・ダメージ・地形破壊範囲は維持。

## v2.0.63 クールカイ連射・表示調整（2026-08-18）

- クールカイの透明余白を表示時に切り出し、選択画面・バトル画面で他キャラと揃うサイズに調整。
- 必殺「Amour 握り飯」をランダム角度の47発連射に変更。0.075秒間隔で順番に発射し、1発2ダメージと地形破壊は維持。
- 専用BGMをタイトル前から先読みするよう変更。
...
- 必殺技名を `Amour 握り飯`、説明を「手燭の油で作った47個の握り飯を配ってやる。」へ変更した。
- 握り飯を大きめの固定微バラツキで47発発射し、1発2ダメージと小さな地形破壊を持たせた。
- `SIX ÉTERNEL ―愛はひとつじゃない―.mp3` を同梱し、クールカイ参加時のBATTLE BGMへ固定した。

### 実測テスト数・確認

- Regression **406/406 passed**。
- 専用BGMのキャラ選択テストを追加した。

## v2.0.60 クールカイ追加（2026-08-18）

### 何をしたか

- クールカイを追加。小さいおにぎりを固定の微バラツキで47発連射する、規格外のネタキャラ。
- キャラクター画像のマスターPNGと実行用WebPを追加し、Firebase Rulesの許可リストを更新した。
- 必殺弾は地形を削らず、各弾1ダメージの直撃専用として通信対戦でも端末差が出ない固定角度列にした。

### なぜしたか

- Google Driveで受け取ったクールカイの素材を、カタモンの新しいネタキャラとして遊べるようにするため。

### やってはいけないこと

- おにぎり47発の固定数・微バラツキ・地形非破壊・1発1ダメージを変更しない。

### 実測テスト数・確認

- 専用Regression 2件を追加し、Regressionは **405/405 passed**。
- 最終の `npm.cmd test` は全件成功（Stage 3 **484/484**、Stage **53/53**）。
...
# v2.0.72 起動ローダーの進捗ゲージ化（2026-08-18）

- 起動時の石壁ローダーを、読み込み項目数ではなく0〜100%の進捗ゲージで表示するよう変更。
- ゲージ上へ起動ごとにランダムなキャラを配置し、進捗に合わせてカタカタ揺れながら進む演出を追加。
# v2.0.73 BGM再生曲名表示（2026-08-18）

- クール=カイ専用曲「SIX ÉTERNEL ―愛はひとつじゃない―」をタイトルの「おまけ5」へ追加。
- タイトル画面とBATTLE STARTロゴ画面へ、現在のBGMを「♪曲名」で表示。
# v2.0.74 ローダーキャラの優先表示（2026-08-18）

- ロード画面の抽選キャラを優先読み込みし、待ち時間は到着済みキャラを表示してボール表示を抑制。
- 左向き素材も右向きへ反転し、ローダー上の向きを統一。

## v2.0.97 オンライン入室直後の誤退出防止（2026-08-21）

- 過去のロビー名簿通知だけで自席喪失と確定せず、Firebaseの現在の席名簿を再確認してから退出するよう修正。
- 同時の確認通信は1本に抑え、通信失敗を席喪失として扱わない既存方針を維持。
- 最新`origin/master`統合後の`npm test`は全工程成功（Stage3 488/488）。実機3参加者QAは公開前に実施する。

## ランキング管理の非表示フラグ（2026-08-27移管）

- `gas/ranking.gs` の `hidden` 列へ `TRUE` を入力した行は、ゲームのランキング表示から除外する。空欄・`FALSE`・`0`は従来どおり表示する。
- GAS側の変更はApps Scriptへ全量貼り付けて新バージョンをデプロイするまで公開環境へ反映されない。
