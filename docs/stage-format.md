...
色は`#rrggbb`形式。`materials`は1件だけを許可する。通常地形に加え、次の鋼鉄を選べる。
...
鋼鉄はステージ全体の地形素材として使う。砲弾は鋼鉄へ当たって爆発するが、地形の形と当たり判定は削れない。キャラクターは通常どおり上に乗れる。通常地形と鋼鉄を同じステージ内で混ぜて配置する機能、氷、沼、バウンド、ダメージ地形は未対応。
